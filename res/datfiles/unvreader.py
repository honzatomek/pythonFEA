...
class Unv_process:
    ...
    def __extract_vertice__(self):
        ...
        for i, vertice_line in enumerate(self.vertices_coordinates[1::2]):
            ...
            coords = [float(c.replace('D', 'E')) for c in coords]  # Transform the coordinate text in float

            # Put the coordinate in the vertices
            self.vertices[i] = np.array(coords, dtype=float)
    ...
    def __extract_faces__(self):
        ...
        for i, faces_line in enumerate(self.faces_coordinates[1::2]):
            ...
            fa = np.array(fa, dtype=float)  # Transform the coordinate text in float

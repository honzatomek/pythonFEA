...
%  Input:
%
%    character TRANSA, specifies the form of op( A ) to be used in
%    the matrix multiplication as follows:
%    'N' or 'n', op ( A ) = A.
%    'T' or 't', op ( A ) = A'.
%    'C' or 'c', op ( A ) = A'.
%
%    character TRANSB, specifies the form of op ( B ) to be used in
%    the matrix multiplication as follows:
%    'N' or 'n', op ( B ) = B.
%    'T' or 't', op ( B ) = B'.
%    'C' or 'c', op ( B ) = B'.
%
%    integer M, the number of rows of the  matrix op ( A ) and of the  
%    matrix C.  0 <= M.
%
%    integer N, the number  of columns of the matrix op ( B ) and the 
%    number of columns of the matrix C.  0 <= N.
%
%    integer K, the number of columns of the matrix op ( A ) and the 
%    number of rows of the matrix op ( B ).  0 <= K.
%
%    real ALPHA, the scalar multiplier 
%    for op ( A ) * op ( B ).
%
%    real A(LDA,KA), where:
...
%
%    integer LDA, the first dimension of A as declared in the calling 
...
%
%    integer LDB, the first dimension of B as declared in the calling 
%    routine.  When TRANSB = 'N' or 'n' then LDB must be at least max ( 1, K ), 
%    otherwise LDB must be at least max ( 1, N ).
%
%    real BETA, the scalar multiplier for C.
%
%    real C(LDC,N).
...
%    on entry.
%
%    integer LDC, the first dimension of C as declared in the calling 
%    routine.  max ( 1, M ) <= LDC.
%
%  Output:
%
%    real C(LDC,N), the array C is overwritten by the M by N matrix

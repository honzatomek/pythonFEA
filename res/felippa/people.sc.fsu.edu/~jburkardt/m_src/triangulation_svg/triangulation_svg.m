...
%
%    integer ELEMENT_ORDER, the order of the element.
%
%    integer ELEMENT_NUM, the number of elements.
%
...
%  Write that junk.
%
  fprintf ( '<?xml version = "1.0" standalone="no"?>\n' );
  fprintf ( '\n' );
  fprintf ( '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN"\n' );
  fprintf ( '  "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n' );
  fprintf ( '\n' );
  fprintf ( '<svg \n' );
  fprintf ( '  width="%d" \n', i4_max );
  fprintf ( '  height="%d" \n', j4_max );
  fprintf ( '  viewbox="%d %d %d %d"\n', ...
    i4_min, j4_min, i4_max, j4_max );
  fprintf ( '  xmlns="http://www.w3.org/2000/svg" \n' );
  fprintf ( '  version="1.1">\n' );
  fprintf ( '  <desc> \n' );
  fprintf ( '    Triangulation created by triangulation_svg.m\n' );
  fprintf ( '  </desc>\n' );
  ...
  for j = 1 : element_num
    ...
    fprintf ( '  <polygon \n' );
    fprintf ( '    fill="red" \n' );
    fprintf ( '    stroke="black" \n' );
    fprintf ( '    stroke-width="2" \n' );
    fprintf ( '    points=" \n' );

...
%
%    real CENTER(3,1), the center of the sphere.
%
...
%
  ...
  p1(1:3,1) = center(1:3,1);
  p2(1:3,1) = center(1:3,1);

  n2 = 0;

  for i = 1 : n

    if ( p(1:3,i) == center(1:3,1) )

    else

      p1(1:3,1) = p2(1:3,1);

      alpha = sqrt ( sum ( ( p(1:3,i) - center(1:3,1) ).^2 ) );

      p2(1:3,1) = center(1:3,1) + r * ( p(1:3,i) - center(1:3,1) ) / alpha;
%
...
%
      elseif ( 1 <= n2 )
        ...
        dot = ( p1(1:3,1) - center(1:3,1) )' * ( p2(1:3,1) - center(1:3,1) );
...
%
          if ( thetamax < abs ( ang3d ) )
            ...
            for j = 1 : nfill-1
              ...
              pi(1:3,1) = ( ( nfill - j ) * ( p1(1:3,1) - center(1:3,1) ) ...
                          + (         j ) * ( p2(1:3,1) - center(1:3,1) ) );

              tnorm = sqrt ( sum ( pi(1:3,1).^2 ) );

              if ( tnorm ~= 0.0 )
                pi(1:3,1) = center(1:3,1) + r * pi(1:3,1) / tnorm;
                n2 = n2 + 1;
                pp(1:3,n2) = pi(1:3,1);
              end

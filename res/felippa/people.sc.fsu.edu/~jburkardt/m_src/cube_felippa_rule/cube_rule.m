...
%  Input:
%
%    real A(3), B(3), the lower and upper limits.
%
%    integer ORDER_1D(3), the order of the rule in each
%    dimension.  1 <= ORDER_1D(I) <= 5.
%
%  Output:
%
%    real W(ORDER_1D(1)*ORDER_1D(2)*ORDER_1D(3)), the weights.
%
%    real XYZ(3,ORDER_1D(1)*ORDER_1D(2)*ORDER_1D(3)), the abscissas.
%

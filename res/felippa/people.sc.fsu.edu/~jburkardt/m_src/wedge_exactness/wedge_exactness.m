function wedge_exactness ( quad_filename, degree_max )
...
%
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'WEDGE_EXACTNESS\n' );
  fprintf ( 1, '  MATLAB/Octave version %s\n', version ( ) );
  fprintf ( 1, '  Investigate the polynomial exactness of\n' );
  fprintf ( 1, '  a quadrature rule for the unit wedge.\n' );
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'WEDGE_EXACTNESS: User input:\n' );
...
%
  ...
  if ( dim_num ~= 3 )
    fprintf ( 1, '\n' );
    fprintf ( 1, 'WEDGE_EXACTNESS - Fatal error!\n' );
    fprintf ( 1, '  The quadrature abscissas must be 3 dimensional.\n' );
    error ( 'WEDGE_EXACTNESS - Fatal error!' );
  end
...
%
  ...
  for degree = 0 : degree_max
    ...
    while ( true )
      ...
      quad = wedge01_volume ( ) * w(1:order)' * v(1:order);

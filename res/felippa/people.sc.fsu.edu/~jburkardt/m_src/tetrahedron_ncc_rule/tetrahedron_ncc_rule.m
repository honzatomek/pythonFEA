function [ xyz, w ] = tetrahedron_ncc_rule ( rule, order_num )
...
%  Output:
%
%    real XYZ(3,ORDER_NUM), the points of the rule.
%
%    real W(ORDER_NUM), the weights of the rule.
%
...
%
  ...
  [ suborder_xyz, suborder_w ] = tetrahedron_ncc_subrule ( rule, suborder_num );
%
...
%
    elseif ( suborder(s) == 24 )
      ...
      fprintf ( 1, '\n' );
      fprintf ( 1, 'TETRAHEDRON_NCC_RULE - Fatal error!\n' );
      fprintf ( 1, '  Illegal SUBORDER(%d) = %d\n', s, suborder(s) );
      error ( 'TETRAHEDRON_NCC_RULE - Fatal error!' );

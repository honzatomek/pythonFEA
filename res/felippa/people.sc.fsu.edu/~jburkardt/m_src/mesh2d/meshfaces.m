...
tic
[qtree.p,qtree.t,qtree.h] = quadtree(node,edge,hdata,options.dhmax,options.stats);
t_quad = toc;
%
% Discretise edges
%
pbnd = boundarynodes(qtree.p,qtree.t,qtree.h,node,edge,options.stats);
...
%% boundarynodes() discretises the geometry based on the edge size requirements interpolated 
...
%% getoptions() extracts the user defined options
...
%% checkposscalar(): Helper function to check if var is a positive scalar.
...
%% checklogicalscalar(): Helper function to check if var is a logical scalar.

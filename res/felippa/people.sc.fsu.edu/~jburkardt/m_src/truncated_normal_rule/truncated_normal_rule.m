...
%  Input:
%
%    integer option: 0/1/2/3 for none, lower, upper, double truncation.
%
%    integer N, the number of points in the rule;
%
%    real MU, the mean of the original normal distribution;
%
%    real SIGMA, the standard deviation of the original normal distribution,
%
%    real A, the left endpoint (for options 1 or 3)
%
%    real B, the right endpoint (for options 2 or 3);
%
...
%    integer N, the order of the matrix.
%
%    real A(N,N), the matrix, which must be square, real,
%    and symmetric.
%
...
%  Input:
%
%    integer M, the number of moments desired.
%
%    real MU, SIGMA, the mean and standard deviation.
%
%  Output:
%
%    real W(M), the weighted integrals of X^0 through X^(M-1).
%
  w = zeros ( m, 1 );
...
%  Output:
%
%    real VALUE, the number of combinations of N
...
%  Output:
%
%    real MOMENT_PDF, the moment of the PDF.
%
  if ( order < 0 )
    fprintf ( 1, '\n' );

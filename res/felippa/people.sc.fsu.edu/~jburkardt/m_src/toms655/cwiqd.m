...
%  Input:
%
%    integer M, the multiplicity of the knot in question.
%
%    integer NM, is equal to max ( N - M, 1 ), where N is
%    the number of knots used, counted according to multiplicity.
%
%    integer L, min ( M, N - M + 1), where N is the number
%    of knots used, counted according to multiplicity.
%
%    real V,  the knot in question.
%
%    real XK(NM), all but the last m entries in the
%    diagonal of K-hat.
%
%    integer NSTAR, the dimension of the Jacobi matrix.
%
%    real PHI(NSTAR), the eigenvalues of the Jacobi matrix.
%
%    real A(NSTAR), the square of the first row of the
%    orthogonal matrix that diagonalizes the Jacobi matrix.
%
%    real R(L), used to compute the right principal vectors.
%
%  Output:
%
%    real D(M), the weights.
%
  wf = zeros ( nstar, 1 );
%

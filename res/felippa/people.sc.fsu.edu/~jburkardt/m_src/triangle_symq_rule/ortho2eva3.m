...
%  Input:
%
%    integer MMAX, the maximum order to which the polynomials are
%    to be evaluated.
%
%    real Z(2), the location where the polynomials are
%    to be evaluated; normally, expected to be inside (including boundary) the
%    reference triangle.
%
%  Output:
%
%    real POLS((mmax+1)*(mmax+2)/2), the orthogonal
%    polynomials evaluated at the point Z.
%
%    real DERSX((mmax+1)*(mmax+2)/2), the derivatives
%    with respect to X of the polynomials returned in array POLS.
%
%    real DERSY((mmax+1)*(mmax+2)/2), the derivatives

...
%  Output:
%
%    integer INT_NUM, the number of intersection
%    points returned.  This will be 0, 1, 2, 3 or 4.
%
%    real PINT(3,4), the coordinates of the
%    intersection points.
%
  int_num = 0;
  pint(1:3,1:4) = 0.0;
...
%
  if ( all ( d(1:4) < 0.0 ) | all ( 0.0 < d(1:4) ) )
    int_num = 0;
    return
  end
%
...
%
  if ( int_num == 4 )
    area1 = quadrilateral_area_3d ( pint );
    pint2 = pint;
    pint2(1:3,3) = pint(1:3,4);
    pint2(1:3,4) = pint(1:3,3);
    area2 = quadrilateral_area_3d ( pint2 );
    if ( area1 < area2 )
      pint = pint2;
    end
  end

...
%
%    real NORMAL(3), a normal vector to the plane.
%
%    real P1(3), P2(3), two distinct points on the line.
%
%  Output:
%
%    integer IVAL, the kind of intersection;
...
%
%    real PINT(3), the coordinates of a
...
%
  direction(1:dim_num,1) = p2(1:dim_num,1) - p1(1:dim_num,1);
  temp = sqrt ( sum ( direction(1:dim_num,1).^2 ) );
  direction(1:dim_num,1) = direction(1:dim_num,1) / temp;
%
%  If the normal and direction vectors are orthogonal, then
%  we have a special case to deal with.
%
  if ( normal(1:dim_num,1)' * direction(1:dim_num,1) == 0.0 )
...
%
  ...
  ival = 1;
  pint(1:dim_num,1) = p1(1:dim_num,1) + temp * direction(1:dim_num,1);

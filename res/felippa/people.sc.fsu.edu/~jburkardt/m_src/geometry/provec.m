...
%  Input:
%
%    integer M, the dimension of the higher order space.
%
%    integer N, the dimension of the lower order space.
%
%    real BASE(M,N).  The columns of BASE contain
%    N vectors, each of length M, which form the basis for
%    a space of dimension N.
%
%    real VECM(M), is an M dimensional vector.
%
%  Output:
%
%    real VECN(N), the projection of VECM into the
%    lower dimensional space.  These values represent
%    coordinates in the lower order space.
%
%    real VECNM(M), the projection of VECM into the

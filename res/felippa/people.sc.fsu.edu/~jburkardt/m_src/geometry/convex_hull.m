function [ hull_num, hull ] = convex_hull ( node_num, node_xy )

%*****************************************************************************80
%
%% convex_hull() computes the convex hull of a set of points in 2D.
%
%  Licensing:
%
...
%  Output:
%
%    integer HULL_NUM, the number of nodes that lie on the convex hull.
%
%    integer HULL(HULL_NUM).  The first HULL_NUM entries contain
  ...
  while ( true )
    ...
    for i = 1 : node_num
      ...
      if ( i ~= q & ...
        ...
        angle = angle_rad ( p_xy, q_xy, node_xy(1:2,i) );

        if ( r == 0 | angle_max < angle )

          r = i;
          r_xy(1:2,1) = node_xy(1:2,r);
          angle_max = angle;
%
...
%
    ...
    if ( node_num < hull_num )
      fprintf ( 1, '\n' );
      fprintf ( 1, 'convex_hull(): Fatal error!\n' );
      fprintf ( 1, '  The algorithm failed.\n' );
      error ( 'convex_hull(): Fatal error!' );
    end
%

function [ xd, td ] = triangulation_order3_sample ( node_num, node_xy, ...
  triangle_num, triangle_node, num_ran )
...
%
%    real NODE_XY(2,NODE_NUM), the node coordinates.
%
%    integer TRIANGLE_NUM, the number of triangles.
%
%    integer TRIANGLE_NODE(3,TRIANGLE_NUM), the nodes that make up the triangles.
%
%    integer NUM_RAN, the number of points to sample.
%
%  Output:
%
%    real XD(2,NUM_RAN), the sample points.
%
%    integer TD(NUM_RAN), the triangle to which each sample point
%    belongs.
%
  dim_num = 2;
%
...
%
  for i = 1 : num_ran
    ...
    xd(1:dim_num,i) = triangle_sample ( t, 1 );

function hypercube_exactness ( quad_filename, degree_max )

%*****************************************************************************80
%
%% hypercube_exactness() tests the exactness of hypercube quadrature.
%
%  Licensing:
%
...
%  Input:
%
%    character QUAD_FILENAME, the file prefix for the quadrature data.
%
%    integer DEGREE_MAX, the maximum degree to check.
%
  timestamp ( );
  fprintf ( 1, '\n' );
  fprintf ( 1, 'HYPERCUBE_EXACTNESS\n' );
...
%
  if ( 1 <= nargin )
%
%  A commandline argument can be blank.  But to let a user
%  specify the shortcut names, also let "D" indicate them.
%
    if ( quad_filename == 'D' )
      quad_filename = '';
    end
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'HYPERCUBE_EXACTNESS: User input:\n' );
...
%
  volume = abs ( prod ( x_range(1:dim_num,2) - x_range(1:dim_num,1) ) );
  w(1:point_num) = w(1:point_num) / volume;
...
%
  ...
  for degree = 0 : degree_max
    ...
    while ( true )
      ...
      quad_error = monomial_quadrature ( dim_num, expon, point_num, w, x );
...
%  Output:
%
%    real VALUE, the value of the integral of the
...
function quad_error = monomial_quadrature ( dim_num, expon, point_num, w, x )
...
%
%    real W(POINT_NUM), the quadrature weights.
%

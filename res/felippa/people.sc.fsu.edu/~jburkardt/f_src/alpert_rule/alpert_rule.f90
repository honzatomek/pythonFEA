...
!  Input:
!
!    integer RULE, the index of the rule, between 1 and 10.
!
!  Output:
!
!    integer A, the value of A.
!
  implicit none

  integer a
  integer, dimension ( 10 ) :: a_vec = (/ &
    1, 2, 2, 3, 3, &
    5, 6, 7, 9, 10 /)
  integer rule
...
!  Input:
!
!    integer RULE, the index of the rule, between 1 and 12.
!
!  Output:
!
!    integer A, the value of A.
...
!  Output:
!
!    real ( kind = rk ) VALUE, the integral of the test integrand 
!    from 0 to 1.
!
!
...
!  Output:
!
!    real ( kind = rk ) F(N), the integrand at the evaluation points.
!
  implicit none
...
!  Output:
!
!    integer J, the value of J.
!
  implicit none
 
  integer j
  integer, dimension ( 10 ) :: j_vec = (/ &
     1,  2,  3,  4,  5, &
     7, 10, 11, 14, 15 /)
  integer rule
...
!  Output:
!
!    integer NUM, the number of rules.
!
  implicit none
...
!
  ...
  integer order
  integer, dimension ( 10 ) :: order_vec = (/ &
     2,  3,  4,  5,  6, &
     8, 10, 12, 14, 16 /)
  integer rule
...
!  Output:
!
!    real ( kind = rk ) ORDER, the order of the rule.
!
  implicit none
  ...
  real ( kind = rk ) order
  real ( kind = rk ), dimension ( 12 ) :: order_vec = (/ &
     1.5,  2.0,  2.5,  3.0,  3.5, &
...
!
  ...
  integer order
  integer, dimension ( 12 ) :: order_vec = (/ &
     3,  4,  5,  6,  7, &
     8, 12, 16, 20, 24, &
    28, 32 /)
  integer rule
...
!    integer RULE, the index of the rule, between 1 and 10.
!
!    integer J, the number of points in the rule.
!
!  Output:
!
!    real ( kind = rk ) X(J), W(J), the points and weights for the rule.
!
  implicit none

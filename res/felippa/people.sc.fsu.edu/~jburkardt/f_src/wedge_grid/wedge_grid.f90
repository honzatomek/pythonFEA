...
!    NG =(N+1)*((N+1)*(N+2))/2.
!
!    Output, real ( kind = rk ) G(3,NG), the coordinates
!    of the grid points.
!
  implicit none
  ...
  do k = 0, n
    kr = real ( 2 * k - n, kind = rk ) / nr
    do j = 0, n
      jr = real ( j, kind = rk ) / nr
      do i = 0, n - j
        ir = real ( i, kind = rk ) / nr
        p = p + 1
        g(1,p) = ir
        g(2,p) = jr
        g(3,p) = kr
...
!    Input, integer N, the number of subintervals.
!
!    Input, integer NG, the number of nodes.
!
!    Input, real ( kind = rk ) G(3,NG), the grid point coordinates.
!
...
!  Parameters:
!
!    Output, real ( kind = rk ) V1(3), V2(3), V3(3), V4(3), V5(3), V6(3),
!    the vertices.
!
  implicit none

...
!
!    Output, real ( kind = rk ) XYZ(3), coordinates of the point.
!
  implicit none
  ...
  if ( i == 0 ) then
    ...
    xc = tan ( real ( 2 * i - n, kind = rk ) * 0.25D+00 * pi &
      / real ( n, kind = rk ) )
  end if
  ...
  if ( j == 0 ) then
    ...
    yc = tan ( real ( 2 * j - n, kind = rk ) * 0.25D+00 * pi &
      / real ( n, kind = rk ) )
  end if
  ...
  if ( k == 0 ) then
    ...
    zc = tan ( real ( 2 * k - n, kind = rk ) * 0.25D+00 * pi &
      / real ( n, kind = rk ) )
  end if
...
!
!    Output, real ( kind = rk ) XYZ(3,NS), distinct points on the unit sphere
...
!
  ...
  do edge = 1, edge_num
    ...
    do f = 1, factor - 1
      ...
      node_xyz(1:3,node) = &
        ( real ( factor - f, kind = rk ) * point_coord(1:3,a)   &
        + real (          f, kind = rk ) * point_coord(1:3,b) ) &
        / real ( factor,     kind = rk )
  ...
  do face = 1, face_num
    ...
    do f1 = 1, factor - 1
      do f2 = 1, factor - f1 - 1
        ...
        node_xyz(1:3,node) = &
          ( real ( factor - f1 - f2, kind = rk ) * point_coord(1:3,a)   &
          + real (          f1,      kind = rk ) * point_coord(1:3,b)   &
          + real (               f2, kind = rk ) * point_coord(1:3,c) ) &
          / real ( factor,           kind = rk )
...
!
    do f = 1, factor - 1
      ...
      angle = ( real ( f, kind = rk ) * theta ) / real ( factor, kind = rk )
...
!
      angle = ( real ( fa, kind = rk ) * theta_ab ) / real ( factor, kind = rk )
      ab(1:3) = cos ( angle ) * point_coord(1:3,a) + sin ( angle ) * bn(1:3)

      angle = ( real ( fa, kind = rk ) * theta_ac ) / real ( factor, kind = rk )
...
!
      do fbc = 1, fa - 1
        ...
        angle = ( real ( fbc, kind = rk ) * theta_bc ) &
                / real ( fa, kind = rk )
...
!
          if ( theta_max < abs ( ang3d ) ) then
            ...
            do j = 1, nfill-1
              ...
              pd(1:3) = &
                ( real ( nfill - j, kind = rk ) * ( p1(1:3) - pc(1:3) ) &
                + real (         j, kind = rk ) * ( p2(1:3) - pc(1:3) ) )
...
!
    do long = 0, long_num - 1
      ...
      theta = real ( long,     kind = rk ) * 2.0D+00 * pi &
...
!
!    Output, real ( kind = rk ) P(3,N), the grid points.
!
  implicit none
...
subroutine sphere_unit_sample ( n, x )

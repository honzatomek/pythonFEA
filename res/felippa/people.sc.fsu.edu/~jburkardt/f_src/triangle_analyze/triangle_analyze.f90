...
!
  ...
  real ( kind = rk ) angles(3)
  real ( kind = rk ) area
  integer arg_num
  real ( kind = rk ) centroid(2)
  real ( kind = rk ) circum_center(2)
  real ( kind = rk ) circum_radius
  integer dim_num
  real ( kind = rk ) edge_length(3)
  logical flag
  integer iarg
  integer iargc
  real ( kind = rk ) in_center(2)
  real ( kind = rk ) in_radius
  character ( len = 255 ) node_filename
  integer node_num
  real ( kind = rk ) node_xy(2,3)
  integer orientation
  real ( kind = rk ) ortho_center(2)
  ...
  write ( *, '(a)' ) '  Use TRIANGLE_PROPERTIES library to analyze triangle read from file.'
!
...
!
  ...
  if ( dim_num /= 2 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRIANGLE_ANALYZE - Fatal error!'
...
!  CIRCUMCIRCLE
!
  call triangle_circumcircle ( node_xy, circum_radius, circum_center )

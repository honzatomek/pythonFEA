...
!
!    Input, real ( kind = rk ) REF(2,N), points in the reference element.
!
...
!
  ...
  do i = 1, f_num
    result(i) = area * sum ( fp(i,1:p_num) ) / real ( p_num, kind = rk )
  end do
...
!    number generator.
!
!    Output, real ( kind = rk ) P(2,P_NUM), the points.
!
  implicit none

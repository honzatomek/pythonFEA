...
!*****************************************************************************80
!
!! r8_besj0() computes J Bessel functions of order 0.
!
!  Licensing:
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argment of the Bessel function.
!
!    Output, real ( kind = rk ) r8_besj0, the value of the Bessel function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) r8_besj0
  integer jint
  real ( kind = rk ) result
  real ( kind = rk ) x
...
!*****************************************************************************80
!
!! r8_besy0 computes Y Bessel functions of order 0.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    Input, real ( kind = rk ) X, the argment of the Bessel function.
!
!    Output, real ( kind = rk ) r8_besy0, the value of the Bessel function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) r8_besy0
  integer jint
  real ( kind = rk ) result
  real ( kind = rk ) x
...
!  Parameters:
!
!    Input, real ( kind = rk ) ARG, the argument of the Bessel function.
!
!    Output, real ( kind = rk ) RESULT, the value of the Bessel function.
!
!    Input, integer JINT, is 0 for the J0 Bessel function, and
...
!
  ...
  real ( kind = rk ) wsq
  real ( kind = rk ) xden
  real ( kind = rk ), parameter :: xmax = 8.19D+03
  real ( kind = rk ) xnum
  real ( kind = rk ), parameter :: xsmall = 1.22D-09
  real ( kind = rk ) xj0
  real ( kind = rk ) xj1
  real ( kind = rk ) xj01
  real ( kind = rk ) xj02
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the value at which the series
!    is to be evaluated.
!    X should be in the range -1.0 <= X <= 1.0.  The routine will refuse
!    to proceed if X <= -1.1 or 1.1 <= X.
!
!    Input, real ( kind = rk ) CS(N), the coefficients of the Chebyshev series.
!    In evaluating CS, only half the first coef is summed.
!
!    Output, real ( kind = rk ) VALUE, the value of the Chebyshev series at X.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) EULER_CONSTANT, the value of the
...
!  Parameters:
!
!    Input, real ( kind = rk ) OS(NOS), the array of coefficients.
!
!    Input, integer NOS, the number of coefficients.
!
!    Input, real ( kind = rk ) ETA, the requested accuracy.
...
!
  ...
  real ( kind = rk ) a
  real ( kind = rk ) b
  real ( kind = rk ) fx(int_num)
  integer i
  integer prob
  real ( kind = rk ) result
  real ( kind = rk ) x(int_num)
  ...
  if ( int_num == 1 ) then
    ...
    do i = 1, int_num
      x(i) = ( real ( int_num - i,     kind = rk ) * a    &
             + real (           i - 1, kind = rk ) * b  ) &
             / real ( int_num     - 1, kind = rk )
    end do
  end if
...
!
  ...
  integer, parameter :: gauss_num = 4

  real ( kind = rk ) a
  real ( kind = rk ) a_sub
  real ( kind = rk ) b
  real ( kind = rk ) b_sub
  real ( kind = rk ) fx(gauss_num)
  real ( kind = rk ), parameter :: gauss_abs(gauss_num) = (/ &
    -0.861136311594052575223946488893D+00, &
    -0.339981043584856264802665759103D+00, &
     0.339981043584856264802665759103D+00, &
     0.861136311594052575223946488893D+00 /)
  real ( kind = rk ), parameter :: gauss_weight(gauss_num) = (/ &
  ...
  integer prob
  real ( kind = rk ) result
  real ( kind = rk ) x(gauss_num)

  call p00_lim ( prob, a, b )

  h = ( b - a ) / real ( int_num, kind = rk )

  result = 0.0D+00

  do int_i = 1, int_num

    a_sub = ( real ( int_num - int_i + 1, kind = rk ) * a   &
            + real (           int_i - 1, kind = rk ) * b ) &
            / real ( int_num,             kind = rk )

    b_sub = ( real ( int_num - int_i, kind = rk ) * a   &
            + real (           int_i, kind = rk ) * b ) &
            / real ( int_num,         kind = rk )
...
!
  ...
  result =  ( b - a ) * sum ( fx(1:int_num) ) / real ( int_num, kind = rk )
...
!  Parameters:
!
!    Output, integer PROB_NUM, the number of test integration
!    problems.
!
  implicit none
...
!
  ...
  do int_i = 1, int_num
    ...
    b_sub = ( real ( int_num - int_i ) * a &
            + real (           int_i ) * b ) / real ( int_num, kind = rk )
...
!
  ...
  fx(1:n) = 2.0D+00 / ( 2.0D+00 + sin ( 10.0D+00 * r8_pi * x(1:n) ) )
...
!    small.  Some compilers may produce code that fails in these cases.
...
!
  ...
  real ( kind = rk ) x(n)
  real ( kind = rk ), parameter :: r8_pi = 3.141592653589793D+00
  real ( kind = rk ), save :: x_max = 0.0D+00
  ...
  do i = 1, n
    ...
    if ( x_max < abs ( x(i) ) ) then
      ...
      fx(i) = sqrt ( 50.0D+00 ) * exp ( - 50.0D+00 * r8_pi * x(i) * x(i) )
    end if
...
!
  ...
  fx(1:n) = 50.0D+00 / r8_pi / ( 2500.0D+00 * x(1:n) * x(1:n) + 1.0D+00 )
...
!
  ...
  fx(1:n) = ( sin ( 50.0D+00 * r8_pi * x(1:n) ) )**2
...
!
  ...
  do i = 1, n
    ...
    fx(i) = 0.0D+00
    do j = 1, n_term
      fx(i) = fx(i) + cos ( 7.0D+00 ** j * r8_pi * x(i) ) / 2.0D+00 ** j
    end do
...
!
  ...
  real ( kind = rk ), save :: alpha = -0.9D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!  Parameters:
!
!    Output, real ( kind = rk ) ALPHA, the current value of the parameter.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) ALPHA, the new value of the parameter.
!
  implicit none
...
!
  ...
  do i = 1, n
    ...
    fx(i) = 4.0D+00**( -alpha ) &
      / ( ( x(i) - 0.25D+00 * r8_pi )**2 + 16.0D+00 ** (-alpha) )
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = 5.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = 3.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = -0.5D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  fx(1:n) = ( abs ( x(1:n) - 0.25D+00 * r8_pi ) )**alpha
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = 0.3D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = 2.0D+00
  character ( len = * ) name
  logical s_eqi
  real ( kind = rk ) value
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: alpha = 0.0D+00
  character ( len = * ) name
  real ( kind = rk ), parameter :: r8_pi = 3.141592653589793D+00
  logical s_eqi
...
!
  ...
  do i = 1, n
    ...
    if ( x(i) == 0.0D+00 ) then
      ...
      fx(i) = log ( x(i) ) * sin ( 10.0D+00 * r8_pi * x(i) )
    end if
...
!
  ...
  exact = - ( r8_ci ( 1.0D+00 ) * sin ( 1.0D+00 ) + &
    ( 0.5D+00 * r8_pi - r8_si ( 1.0D+00 ) ) * cos ( 1.0D+00 ) ) / r8_pi
...
!
  ...
  character ( len = * ) action
  real ( kind = rk ), save :: c = 3.00D+00
  character ( len = * ) name
  real ( kind = rk ) value
  real ( kind = rk ), save :: x0 = 0.75D+00
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the point at which the terms are needed.
!    |X| must be at least 4.
!
!    Output, real ( kind = rk ) F, G, the values of F and G at X.
!
  implicit none
  ...
  real ( kind = rk ) eta
  real ( kind = rk ) f
  real ( kind = rk ), parameter, dimension ( 20 ) :: f1cs = (/ &
  ...
  real ( kind = rk ) f2cs(29)
  real ( kind = rk ) g
  real ( kind = rk ) g1cs(21)
  real ( kind = rk ) g2cs(34)
  integer, save :: nf1 = 0
  integer, save :: nf2 = 0
  integer, save :: ng1 = 0
  integer, save :: ng2 = 0
  ...
  real ( kind = rk ) x
  real ( kind = rk ), save :: xbig = 0.0D+00
  real ( kind = rk ), save :: xbnd = 0.0D+00
  real ( kind = rk ), save :: xmaxf = 0.0D+00
  real ( kind = rk ), save :: xmaxg = 0.0D+00
!
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the number to be decomposed.
!
!    Output, real ( kind = rk ) Y, the mantissa of the decomposition.
!
!    Output, integer N, the exponent of the decomposition.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument of the cosine integral.
!
!    Output, real ( kind = rk ) R8_CI, the value of the cosine integral.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  integer, parameter :: ncics = 13

  real ( kind = rk ), parameter, dimension ( ncics ) :: cics = (/ &
...
!    X must be positive.
!
!    Output, real ( kind = rk ) GAMMA_LOG, the logarithm of the Gamma function
...
!
!    Output, real ( kind = rk ) R8_SECH, the value of the function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) :: log_huge = 80.0D+00
  real ( kind = rk ) r8_sech
  real ( kind = rk ) x
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the argument of the sine integral.
!
!    Output, real ( kind = rk ) R8_SI, the value of the sine integral.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  integer, parameter :: nsics = 12
  ...
  real ( kind = rk ), parameter :: r8_pi = 3.141592653589793D+00
  real ( kind = rk ) r8_si
  real ( kind = rk ), save, dimension ( nsics ) :: sics = (/ &
  ...
  if ( absx < xsml ) then
    ...
    call csevl ( ( x * x - 8.0D+00 ) * 0.125D+00, sics, nsi, value )
    r8_si = x * ( 0.75D+00 + value )
  else
    call r89sifg ( absx, f, g )
    r8_si = 0.5D+00 * r8_pi - f * cos ( absx ) - g * sin ( x )
    if ( x < 0.0D+00 ) then
      r8_si = - r8_si
    end if
  end if

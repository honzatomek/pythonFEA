...
!
!    Input, integer TET_NODE(TET_ORDER,TET_NUM), the 
!    indices of the nodes.
!
!    Output, integer TET_NEIGHBOR(4,TET_NUM), the four
...
!  Parameters:
!
!    Input, integer TET_ORDER, the order of the mesh, either 
!    4 or 10.
!
!    Input, integer TET_NUM, the number of tetrahedrons.
!
!    Input, integer TET_NODE(TET_ORDER,TET_NUM), the indices
!    of the nodes.
!
...
!    Input, integer TET_NUM, the number of tetrahedrons.
!
!    Input, integer TET_NODE(4,TET_NUM), the indices of 
!    the nodes.
!
!    Output, integer ADJ_NUM, the total number of adjacency
!    relationships,
!
...
!    the nodes.
!
!    Input, integer ADJ_NUM, the total number of adjacency 
!    relationships,
!
...
!    the nodes.
!
!    Output, integer BOUNDARY_FACE_NUM, the number of boundary 
!    faces.
!
  implicit none
...
!    Input, integer TET_NUM, the number of tetrahedrons.
!
!    Output, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the node coordinates.
!
...
!
  ...
  integer, dimension ( 4, tet_num ) :: tet_node
  real ( kind = rk ), dimension ( 3, node_num ) :: node_xyz
...
!  Parameters:
!
!    Input, integer NODE_NUM1, the number of nodes in the input
!    mesh.
!
!    Input, integer TET_NUM1, the number of tetrahedrons in 
!    the input mesh.
!
!    Input, real ( kind = rk ) NODE_XYZ1(3,NODE_NUM1), the coordinates of
!    the nodes that make up the input mesh.
!
!    Input, integer TET_NODE1(4,TET_NUM1), the indices of 
!    the nodes in the input mesh.
!
!    Input, integer NODE_NUM2, the number of nodes for the 
!    refined mesh.
!
!    Input, integer TET_NUM2, the number of tetrahedrons in the
!    refined mesh.
!
!    Input, integer EDGE_DATA(5,6*TET_NUM), edge data.
!
!    Output, real ( kind = rk ) NODE_XYZ2(3,NODE_NUM2), the coordinates of
!    the nodes that make up the output mesh.
!
!    Output, integer TET_NODE2(4,TET_NUM2), the indices of 
...
!
  ...
  integer n2_old
  integer node
  real ( kind = rk ) node_xyz1(3,node_num1)
  real ( kind = rk ) node_xyz2(3,node_num2)
...
!    original mesh.
!
!    Input, integer TET_NUM1, the number of tetrahedrons in the
!    original mesh.
!
!    Input, integer TET_NODE1(4,TET_NUM1), the indices of 
!    the nodes that form the tetrahedrons in the input mesh.
!
!    Output, integer NODE_NUM2, the number of nodes in the 
!    refined mesh.
!
!    Output, integer TET_NUM2, the number of tetrahedrons in the
!    refined mesh.
!
!    Output, integer EDGE_DATA(5,6*TET_NUM1), edge data.
!
  implicit none
...
!  Parameters:
!
!    Input, integer TET_NUM, the number of tetrahedrons in the
!    linear mesh.
!
!    Input, integer TET_NODE1(4,TET_NUM), the indices of 
!    the nodes in the linear mesh.
!
!    Input, integer NODE_NUM1, the number of nodes for the 
!    linear mesh.
!
!    Input, real ( kind = rk ) NODE_XYZ1(3,NODE_NUM1), the coordinates of
...
!    Input, integer EDGE_DATA(5,6*TET_NUM), edge data.
!
!    Output, integer TET_NODE2(10,TET_NUM), the indices of 
...
!    linear mesh.
!
!    Output, integer EDGE_DATA(5,6*TET_NUM), edge data.
!
!    Output, integer NODE_NUM2, the number of nodes for the
!    quadratic mesh.
!
  implicit none
...
!    to account for symmetry.
!
!    Thanks to Iban Constenia Rozados for an important correction,
!    28 February 2013.
!
!  Licensing:
!
...
!    Input, integer TET_NUM, the number of tetrahedrons.
!
!    Input, integer TET_NODE(10,TET_NUM), the indices of 
!    the nodes.
!
!    Output, integer ADJ_NUM, the total number of adjacency
!    relationships,
...
!    Output, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the node coordinates.
!
!    Output, integer TET_NODE(10,TET_NUM), the nodes 
!    forming each tet.
!
  implicit none
  ...
  integer, dimension ( 10, tet_num ) :: tet_node
  real ( kind = rk ), dimension ( 3, node_num ) :: node_xyz
...
!
!    Input, integer TET_NODE1(10,TET_NUM1), the indices of 
!    the nodes in the quadratic tet mesh.
!
!    Input, integer TET_NUM2, the number of tetrahedrons in 
...
!
!    Input, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates 
!    of the nodes.
!
!    Input, integer TET_ORDER, the order of tetrahedrons in 
!    the tet mesh.
!
!    Input, integer TET_NUM, the number of tetrahedrons in the 
!    tet mesh.
...
!
!    Input, real ( kind = rk ) QUAD_XYZ(3,QUAD_NUM), the abscissas of the 
...
!
!    Output, real ( kind = rk ) REGION_VOLUME, the volume of the region.
!
  implicit none
  ...
  real ( kind = rk ) quad_xyz(3,quad_num)
  real ( kind = rk ) quad2_xyz(3,quad_num)
  real ( kind = rk ) region_volume
  integer tet
  real ( kind = rk ) tet_volume
  integer tet_node(tet_order,tet_num)
  real ( kind = rk ) tet_xyz(3,4)
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the nodes.
!
!    Input, integer TET_ORDER, the order of the mesh, either
!    4 or 10.
!
...
!    indices of the nodes.
!
!    Output, real ( kind = rk ) TET_QUALITY(TET_NUM), the quality
...
!
  ...
  real ( kind = rk ) node_xyz(dim_num,node_num)
  integer tet
  integer tet_node(tet_order,tet_num)
  real ( kind = rk ) tet_quality(tet_num)
...
!    Input, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the nodes.
!
!    Input, integer TET_ORDER, the order of the mesh, 
!    either 4 or 10.
!
!    Input, integer TET_NUM, the number of tetrahedrons.
!
...
!    Input, integer NODE_NUM, the number of nodes.
!
!    Input, real ( kind = rk ) NODE_XYZ(3,NODE_NUM), the coordinates of 
!    the nodes.
!
!    Input, integer TET_ORDER, the order of the tetrahedrons.
!
...
!
!    Input, real ( kind = rk ) P(3), the coordinates of a point.
!
...
!    the point.
!
!    Output, integer STEP_NUM, the number of tetrahedrons checked.
!
  implicit none
...
!    Input, real ( kind = rk ) TET_XYZ(3,4), the coordinates of the vertices.
!
!    Output, real ( kind = rk ) EDGE_LENGTH(6), the length of the edges.
!
  implicit none
...
!
  ...
  real ( kind = rk ) l134
  real ( kind = rk ) l234
  real ( kind = rk ) n123(1:dim_num)
  real ( kind = rk ) n124(1:dim_num)
  real ( kind = rk ) n134(1:dim_num)
  real ( kind = rk ) n234(1:dim_num)
  real ( kind = rk ) pc(1:dim_num)
  real ( kind = rk ) r
  real ( kind = rk ) r8mat_det_4d
  real ( kind = rk ) r8vec_norm
  real ( kind = rk ) tet_xyz(1:dim_num,4)
  real ( kind = rk ) v21(1:dim_num)
  real ( kind = rk ) v31(1:dim_num)
  real ( kind = rk ) v41(1:dim_num)
  real ( kind = rk ) v32(1:dim_num)
  real ( kind = rk ) v42(1:dim_num) 
  real ( kind = rk ) v43(1:dim_num) 
...
!
!    Input, real ( kind = rk ) PHY(3,N), the coordinates of physical points
!    to be transformed.
!
!    Output, real ( kind = rk ) REF(3,N), the coordinates of the corresponding
...
!    Input, real ( kind = rk ) TET_XYZ(3,4), the coordinates of the vertices.
!
!    Output, real ( kind = rk ) QUALITY, the quality of the tetrahedron.
!
  implicit none
...
!    Input, real ( kind = rk ) TET_XYZ(3,4), the coordinates of the vertices.
!
!    Output, real ( kind = rk ) QUALITY2, the quality of the tetrahedron.
!
  implicit none
...
!    Input, real ( kind = rk ) TET_XYZ(3,4), the coordinates of the vertices.
!
!    Output, real ( kind = rk ) QUALITY3, the mean ratio of the tetrahedron.
!
  implicit none
  ...
  real ( kind = rk ) ab(dim_num)
  real ( kind = rk ) ac(dim_num)
  real ( kind = rk ) ad(dim_num)
  real ( kind = rk ) bc(dim_num)
  real ( kind = rk ) bd(dim_num)
  real ( kind = rk ) cd(dim_num)
  real ( kind = rk ) denom
  real ( kind = rk ) lab
  real ( kind = rk ) lac
  real ( kind = rk ) lad
...
!    Input, real ( kind = rk ) TET_XYZ(3,4), the coordinates of the vertices.
!
!    Output, real ( kind = rk ) QUALITY4, the value of the quality measure.
!
  implicit none
  ...
  real ( kind = rk ) bc(dim_num)
  real ( kind = rk ) bd(dim_num)
  real ( kind = rk ) c(dim_num)
  real ( kind = rk ) cd(dim_num)
  real ( kind = rk ) d(dim_num)
  real ( kind = rk ) denom
  real ( kind = rk ) l1
  real ( kind = rk ) l2
  real ( kind = rk ) l3
...
!    number generator.
!
!    Output, real ( kind = rk ) P(3,N), random points in the tetrahedron.
!
  implicit none

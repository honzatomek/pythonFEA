...
!
  ...
  integer j
  real ( kind = rk ) p(2,n)
  real ( kind = rk ) :: p1(2) = (/ 0.0D+00, 1.0D+00 /)
  real ( kind = rk ) :: p2(2) = (/ 5.0D+00, 0.0D+00 /)
  real ( kind = rk ) :: p3(2) = (/ 4.0D+00, 4.0D+00 /)
  integer seed
  real ( kind = rk ) v(m,n)
  real ( kind = rk ) :: v1(m) = (/ 1.0D+00, 0.0D+00, 0.0D+00 /)
  real ( kind = rk ) :: v2(m) = (/ 0.0D+00, 1.0D+00, 0.0D+00 /)
  real ( kind = rk ) :: v3(m) = (/ 0.0D+00, 0.0D+00, 1.0D+00 /)

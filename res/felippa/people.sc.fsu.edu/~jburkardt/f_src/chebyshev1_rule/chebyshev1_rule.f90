...
!*****************************************************************************80
!
!! chebyshev1_rule() computes a standard Gauss-Chebyshev type 1 quadrature rule.
!
!  Discussion:
!
...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'CHEBYSHEV1_RULE'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Compute a Gauss-Chebyshev type 1 rule for'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    Integral ( A <= x <= B ) f(x) ' // &
    '/ sqrt ( ( x - A ) * ( B - x ) ) dx'
  write ( *, '(a)' ) ' '
...
!
  ...
  rule_kind = 2
  call cgqf ( order, rule_kind, alpha, beta, a, b, x, w )
!
!  Write the rule.
!
  r(1) = a
...
!    the weight function specified by rule_kind.
!
...
!    be calculated.  This value is only needed when rule_kind has the value 8.
!
!    Input, real ( kind = rk ) ALPHA, BETA, the parameters, if required
!    by the value of rule_kind.
!
  implicit none
...
!
  if ( 3 <= rule_kind .and. rule_kind <= 8 .and. alpha <= -1.0D+00 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'PARCHK - Fatal error!'
...
!
  if ( rule_kind == 4 .and. rule_kind <= 8 .and. beta <= -1.0D+00 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'PARCHK - Fatal error!'

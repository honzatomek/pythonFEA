...
!    1 <= K.
!
!    Output, integer D, the degree (sum) of the corresponding 
!    Pascal indices.
!
  implicit none
...
!  Parameters:
!
!    Input, integer I, J, the row and column indices.  I and J 
!    must be nonnegative.
!
...
!
!    Input, real ( kind = rk ) P1(M1), the polynomial coefficients.
!    M1 = ((D1+1)*(D1+2))/2.
!
!    Input, integer N, the nonnegative integer power.
!
!    Input, integer D2, the degree of the power polynomial.
!    D2 = N * D1.
!
!    Output, real ( kind = rk ) P2(M2), the polynomial power.
!    M2 = ((D2+1)*(D2+2))/2.
!
  implicit none
  ...
  integer m3
  integer m4
  real ( kind = rk ) p1(( ( d1 + 1 ) * ( d1 + 2 ) ) / 2)
  real ( kind = rk ) p2(( ( d2 + 1 ) * ( d2 + 2 ) ) / 2)
  real ( kind = rk ), allocatable :: p3(:)
  real ( kind = rk ), allocatable :: p4(:)
...
!
!    Input, real ( kind = rk ) P1(M1), the coefficients of the linear polynomial.
!    M1 = ( (D1+1)*(D1+2) ) / 2, which should be 3.
!
!    Input, integer N, the power to which the polynomial is to be 
!    raised.  0 <= N.
!
!    Input, integer D2, the degree of the power polyynomial.
!    D2 = N * D1 = N.
!
!    Output, real ( kind = rk ) P2(M2), the coefficients of the power polynomial.
...
!
        ...
        p2(l) = real ( trinomial ( i, j, k ), kind = rk ) &
...
!
!    Output, real ( kind = rk ) P(M), the coefficients of all monomials of 
...
!
!    Input, real ( kind = rk ) P1(M1), the factor 1 coefficients.
!    M1 = ((D1+1)*(D1+2))/2.
!
!    Input, integer D2, the degree of factor 2.
!
!    Input, real ( kind = rk ) P2(M2), the factor2 coefficients.
!    M2 = ((D2+1)*(D2+2))/2.
!
!    Input, integer D3, the degree of the result.
!    D3 = D1 + D2.
!
!    Output, real ( kind = rk ) P3(M3), the result coefficients.
!    M3 = ((D3+1)*(D3+2))/2.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(2,3), the coordinates of the vertices.  The
!    vertices are assumed to be the images of (0,0), (1,0) and (0,1) 
!    respectively.
!
!    Output, real ( kind = rk ) A, B, C, D, E, F, the mapping coefficients.
!
  implicit none
...
!
!    Output, real ( kind = rk ) TRIANGLE01_MONOMIAL_INTEGRAL, the integral.
!
  implicit none
  ...
  integer l
  real ( kind = rk ) q
  real ( kind = rk ) triangle01_monomial_integral
  ...
  do l = 1, i
    k = k + 1
    q = q * real ( l, kind = rk ) / real ( k, kind = rk )
  end do
...
!    Input, integer D, the degree of the polynomial.
!
!    Input, real ( kind = rk ) P(M), the polynomial coefficients.
!    M = ((D+1)*(D+2))/2.
!
!    Output, real ( kind = rk ) TRIANGLE01_POLY_INTEGRAL, the integral.
!
  implicit none
  ...
  real ( kind = rk ) p(( ( d + 1 ) * ( d + 2 ) ) / 2)
  real ( kind = rk ) q
  real ( kind = rk ) triangle01_poly_integral
  real ( kind = rk ) triangle01_monomial_integral
...
!  Parameters:
!
!    Input, real ( kind = rk ) T(2,3), the vertices of the triangle.
!
!    Output, real ( kind = rk ) TRIANGLE_AREA, the area of the triangle.
!
  implicit none
...
!    Input, real ( kind = rk ) T(2,3), the vertices of the triangle.
!
!    Output, real ( kind = rk ) TRIANGLE_MONOMIAL_INTEGRAL, the integral 
...
!
  ...
  integer, parameter :: d2 = 1

  integer, parameter :: m1 = ( ( d1 + 1 ) * ( d1 + 2 ) ) / 2
  integer, parameter :: m2 = ( ( d2 + 1 ) * ( d2 + 2 ) ) / 2
  ...
  real ( kind = rk ), allocatable :: p3(:)
  real ( kind = rk ), allocatable :: p4(:)
  real ( kind = rk ), allocatable :: p5(:)
  real ( kind = rk ) q
  real ( kind = rk ) t(2,3)
  real ( kind = rk ) triangle_area
  real ( kind = rk ) triangle_monomial_integral
  real ( kind = rk ) triangle01_poly_integral
!
...
!    Input, real ( kind = rk ) T(2,3), the vertices of the triangle.
!
!    Output, real ( kind = rk ) TRIANGLE_POLY_INTEGRAL, the integral.
!
  implicit none
  ...
  real ( kind = rk ) t(2,3)
  real ( kind = rk ) triangle_monomial_integral
  real ( kind = rk ) triangle_poly_integral
...
!  Parameters:
!
!    Input, real ( kind = rk ) X1, Y1, X2, Y2, X3, Y3, the coordinates of the
!    triangle vertices.
!
!    Output, real ( kind = rk ) TRIANGLE_XY_INTEGRAL, the integral of X*Y 
!    over the triangle.
!
  implicit none
  ...
  real ( kind = rk ) q
  real ( kind = rk ) triangle01_monomial_integral
  real ( kind = rk ) triangle_xy_integral
  real ( kind = rk ) x1
  real ( kind = rk ) x2
  real ( kind = rk ) x3
  real ( kind = rk ) y1

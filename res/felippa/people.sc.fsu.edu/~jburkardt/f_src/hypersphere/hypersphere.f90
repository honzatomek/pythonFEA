...
!
!    Input, real ( kind = rk ) C(M), the center of the hypersphere.
!
!    Input, real ( kind = rk ) X(M,N), the Cartesian coordinates of the points.
!
!    Output, real ( kind = rk ) R(N), the radius of the points on the 
!    hypersphere.  Except for the trivial case M = 1, R is assumed nonnegative.
!
!    Output, real ( kind = rk ) THETA(M-1,N), the coordinate angles of the 
...
!
!    Output, real ( kind = rk ) HYPERSPHERE_01_AREA, the area.
!
  implicit none
...
!
!    Output, real ( kind = rk ) AREA, the area
!    in that dimension.
!
  implicit none
...
subroutine hypersphere_01_interior_uniform ( m, n, x )
...
subroutine hypersphere_01_surface_uniform ( m, n, x )
...
!    Input, integer M, the spatial dimension.
!
!    Output, real ( kind = rk ) HYPERSPHERE_01_VOLUME, the volume.
!
  implicit none
...
!
!    Output, real ( kind = rk ) HYPERSPHERE_AREA, the area.
!
  implicit none
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) X(M,N), the points to be mapped.
!
!    Output, real ( kind = rk ) X2(M-1,N), the stereographically mapped points.
!
  implicit none
...
!    Input, integer N, the number of points.
!
!    Input, real ( kind = rk ) X2(M-1,N), points in the plane.
!
!    Input, real ( kind = rk ) X(M,N), points mapped back to the hypersphere.
!
  implicit none
...
subroutine hypersphere_surface_uniform ( m, n, r, c, x )
...
!    Input, real ( kind = rk ) R, the radius.
!
!    Input, real ( kind = rk ) C(M), the center.
!
!    Output, real ( kind = rk ) X(M,N), the points.
!
  implicit none
...
!    Input, real ( kind = rk ) R, the radius.
!
!    Output, real ( kind = rk ) HYPERSPHERE_VOLUME, the volume.
!
  implicit none

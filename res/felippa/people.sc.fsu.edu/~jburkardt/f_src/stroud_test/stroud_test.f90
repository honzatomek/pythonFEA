...
!
  ...
  real ( kind = rk ), dimension ( n_max ) :: center = (/ &
    1.0D+00, -1.0D+00, 2.0D+00 /)
  real ( kind = rk ), external :: function_nd
  character ( len = 7 ) function_nd_name
  integer function_nd_num
  integer i
  integer n
  integer num
  real ( kind = rk ) result1
  real ( kind = rk ) result2
  real ( kind = rk ) r
...
!
  ...
  real ( kind = rk ) ball_volume_nd
  real ( kind = rk ), dimension ( dim_num ) :: center = (/ &
    0.0D+00, 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), external :: mono_000_3d
  real ( kind = rk ), external :: mono_111_3d
  real ( kind = rk ), external :: mono_202_3d
  real ( kind = rk ), external :: mono_422_3d
  integer p(dim_num)
  real ( kind = rk ) result1
  real ( kind = rk ) result2
...
!
  ...
  real ( kind = rk ), external :: function_3d
  integer i
  integer num
  real ( kind = rk ) result1
  real ( kind = rk ) result2
  real ( kind = rk ) result3
...
!
  ...
  real ( kind = rk ), dimension ( dim_num, test_num ) :: center_test = reshape ( (/ &
    0.0D+00, 0.0D+00, &
    0.0D+00, 0.0D+00 /), (/ dim_num, test_num /) )
  real ( kind = rk ) circle_annulus_area_2d
  real ( kind = rk ), external :: function_2d
  ...
  real ( kind = rk ), dimension ( test_num ) :: radius1_test = (/ &
    0.0D+00, 1.0D+00 /)
  real ( kind = rk ) radius2
  real ( kind = rk ), dimension ( test_num ) :: radius2_test = (/ &
    1.0D+00, 2.0D+00 /)
  real ( kind = rk ) result
...
!
  ...
  real ( kind = rk ) ra(5)
  real ( kind = rk ) radius
  real ( kind = rk ) radius1a
  real ( kind = rk ) radius2a
  real ( kind = rk ) radius1b
  real ( kind = rk ) radius2b
  real ( kind = rk ) radius1c
  real ( kind = rk ) radius2c
  real ( kind = rk ) radius1d
  real ( kind = rk ) radius2d
  real ( kind = rk ) result1
  real ( kind = rk ) result2
  integer rule
  real ( kind = rk ) rw(5)
  real ( kind = rk ) ta(20)
  real ( kind = rk ) theta1a
  real ( kind = rk ) theta2a
  real ( kind = rk ) theta1b
  real ( kind = rk ) theta2b
  real ( kind = rk ) theta1c
  real ( kind = rk ) theta2c
  real ( kind = rk ) theta1d
  real ( kind = rk ) theta2d
  real ( kind = rk ) tw(20)
  real ( kind = rk ) zw
...
!
  ...
  real ( kind = rk ), dimension ( dim_num ) :: center = (/ 0.0D+00, 0.0D+00 /)
  real ( kind = rk ), external :: function_2d
  character ( len = 7 ) function_2d_name
  integer function_2d_num
  integer i
...
!
  ...
  do i = 0, 12
    ...
    theta1 = 0.0D+00
    theta2 = real ( i, kind = rk ) * 2.0D+00 * pi / 12.0D+00
...
!
  ...
  real ( kind = rk ), parameter :: pi = 3.141592653589793D+00
  real ( kind = rk ) radius
  real ( kind = rk ), dimension ( test_num ) :: radius_test = (/ &
    1.0D+00, 2.0D+00, 4.0D+00, 8.0D+00 /)
  real ( kind = rk ) result(nrlo:nrhi)
  real ( kind = rk ) theta1
  real ( kind = rk ), dimension ( test_num ) :: theta1_test = (/ &
    0.0D+00, 0.0D+00, 0.0D+00, 0.0D+00 /)
  real ( kind = rk ) theta2
  real ( kind = rk ), dimension ( test_num ) :: theta2_test = (/ &
...
!
  ...
  real ( kind = rk ) result1
  real ( kind = rk ) result2
  real ( kind = rk ) resulta
  real ( kind = rk ) resultb
  integer rule
  real ( kind = rk ) rw(5)
  real ( kind = rk ) ta(20)
  real ( kind = rk ) theta1
...
!
  ...
  integer, parameter :: dim_num = 2
  integer, parameter :: rule_max = 9

  real ( kind = rk ), dimension ( dim_num ) :: center = (/ 1.0D+00, 1.0D+00 /)
  real ( kind = rk ), external :: function_2d
  character ( len = 7 ) function_2d_name
  integer function_2d_num
  integer i
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: ra
  real ( kind = rk ) result(rule_max)
  real ( kind = rk ), allocatable, dimension ( : ) :: rw
  integer rule
  real ( kind = rk ), allocatable, dimension ( : ) :: ta
  real ( kind = rk ), allocatable, dimension ( : ) :: tw
  real ( kind = rk ) zw
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: ytab
...
!
  ...
  if ( d <= p ) then
    ...
    volume_1d = sqrt ( pi ) * gamma ( alpha + 1.0D+00 ) &
      / gamma ( alpha + 1.5D+00 )
    allocate ( x(n,o) )
    allocate ( w(o) )
...
!
  ...
  real ( kind = rk ) beta
  real ( kind = rk ), dimension ( test_num ) :: beta_test = (/ &
  ...
  if ( d <= p ) then
    ...
    volume_1d = 2.0D+00 ** ( alpha + beta + 1.0D+00 ) &
      * gamma ( alpha + 1.0D+00 ) * gamma ( beta + 1.0D+00 ) &
      / ( alpha + beta + 1.0D+00 ) / gamma ( alpha + beta + 1.0D+00 )
    allocate ( x(n,o) )
    allocate ( w(o) )
    call gw_02_xiu ( n, o, gamma0, delta0, c1, volume_1d, x, w )
    allocate ( v(1:o) )
...
!
  ...
  integer num
  real ( kind = rk ) r1
  real ( kind = rk ), dimension ( test_num ) :: r1_test = (/ &
    0.0D+00, 1.0D+00 /)
  real ( kind = rk ) :: r2
  real ( kind = rk ), dimension ( test_num ) :: r2_test = (/ &
    1.0D+00, 2.0D+00 /)
  real ( kind = rk ) result
  integer test
...
!
  ...
  real ( kind = rk ) b1
  real ( kind = rk ) b(3)
  real ( kind = rk ), external :: fl18
  real ( kind = rk ), external :: fl28
  character ( len = 7 ) function_3d_name
  integer function_3d_num
  real ( kind = rk ), external :: fu18
  real ( kind = rk ), external :: fu28
  real ( kind = rk ), external :: function_3d
  integer i
  integer, parameter :: n = 3
  integer num
...
!
  ...
  integer num
  real ( kind = rk ) qa(max_k)
  real ( kind = rk ) qb(max_k)
...
!
  ...
  real ( kind = rk ), dimension ( dim_num ) :: center = (/ &
    0.0D+00, 0.0D+00 /)
  real ( kind = rk ), external :: f_1_2d
  real ( kind = rk ), external :: f_x_2d
  real ( kind = rk ), external :: f_r_2d
  integer i
  integer :: i_max = 8
  ...
  do i = 0, i_max
    ...
    theta1 = 0.0D+00
    theta2 = real ( i, kind = rk ) * 2.0D+00 * pi &
      / real ( i_max, kind = rk )
...
!
  ...
  real ( kind = rk ) result
  real ( kind = rk ), dimension ( npts) :: x = (/ &
    0.0D+00, 1.0D+00, 1.0D+00, 0.0D+00 /)
  real ( kind = rk ), dimension ( npts) :: y = (/ &
...
!
  ...
  integer, dimension ( rule_num ) :: order = (/ &
...
!
  ...
  real ( kind = rk ), external :: function_1d
...
!
  ...
  real ( kind = rk ) result3
  real ( kind = rk ) result4
  real ( kind = rk ) result5
  real ( kind = rk ) result6
  real ( kind = rk ) sphere_unit_area_nd
...
!
  ...
  do i = 0, test_num+1
    ...
    h = 2.0D+00 * r * real ( i, kind = rk ) / real ( test_num, kind = rk )
...
!
  ...
  do dim_num = 2, 5
    ...
    do i = 0, n + 1
      h = real ( 2 * i, kind = rk ) * r / real ( n, kind = rk )
...
!
  ...
  integer, parameter :: dim_num = 2
  integer, parameter :: rule_max = 6

  real ( kind = rk ), dimension ( dim_num ) :: center = (/ 2.0D+00, 2.0D+00 /)
  real ( kind = rk ), external :: function_2d
  character ( len = 7 ) function_2d_name
  integer function_2d_num
  integer i
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: xtab
  real ( kind = rk ), allocatable, dimension ( : ) :: ytab
  real ( kind = rk ), allocatable, dimension ( : ) :: ztab
...
!
  ...
  real ( kind = rk ), allocatable, dimension ( : ) :: weight
  real ( kind = rk ), allocatable, dimension ( : ) :: xtab
  real ( kind = rk ), dimension ( 3 ) :: xval = (/ &
    0.0D+00, 0.0D+00, 1.0D+00 /)
  real ( kind = rk ), allocatable, dimension ( : ) :: ytab
  real ( kind = rk ), dimension ( 3 ) :: yval = (/ &
...
!
  ...
  do a = 0, 10
    ...
    do b = 0, 10 - a
      ...
      coef = real ( a + b + 2, kind = rk ) * real ( a + b + 1, kind = rk )
      do i = 1, b
        coef = coef * real ( a + i, kind = rk ) / real ( i, kind = rk )
      end do
...
!
  ...
  integer nt
  real ( kind = rk ), allocatable, dimension ( : ) :: t
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the value of the variable.
!
!    Output, real ( kind = rk ) FUNCTION_1D, the value of the function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) function_1d
  integer ifunc
  real ( kind = rk ) value
  real ( kind = rk ) x
...
!  Parameters:
!
!    Output, integer FUNCTION_1D_NUM, the number of 1D functions.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, the value of the variables.
!
!    Output, real ( kind = rk ) FUNCTION_2D, the value of the function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) function_2d
  integer ifunc
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!  Parameters:
!
!    Output, integer FUNCTION_2D_NUM, the number of 2D functions.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, Y, Z, the value of the variables.
!
!    Output, real ( kind = rk ) FUNCTION_3D, the value of the function.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) function_3d
  integer ifunc
  real ( kind = rk ) value
  real ( kind = rk ) x
  real ( kind = rk ) y
...
!  Parameters:
!
!    Output, integer FUNCTION_3D_NUM, the number of 3D functions.
!
  implicit none
...
!
!    Output, real ( kind = rk ) FUNCTION_ND, the value of the function.
!
  implicit none
...
!  Parameters:
!
!    Output, integer FUNCTION_ND_NUM, the number of ND functions.
!
  implicit none
...
!
!    Output, real ( kind = rk ) F_1_2D, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, Y, the arguments.
!
!    Output, real ( kind = rk ) F_X_2D, the value of the function.
!
  implicit none
...
!    Input, real ( kind = rk ) X, Y, the arguments.
!
!    Output, real ( kind = rk ) F_R_2D, the value of the function.
!
  implicit none

...
!
!    Input, real ( kind = rk ) C(2), the coordinates of the center of the circle.
!
!    Input, integer NG, the number of grid points, as determined by
!    DISK_GRID_COUNT.
!
!    Output, real ( kind = rk ) CG(2,NG), the grid points inside the circle.
!
  implicit none
...
!    Input, real ( kind = rk ) C(2), the coordinates of the center of the circle.
!
!    Output, real ( kind = rk ) G(2,N), the grid points.
!
  implicit none
  ...
  r0 = r / sqrt ( real ( n, kind = rk ) - 0.5D+00 )
  phi = ( 1.0D+00 + sqrt ( 5.0D+00 ) ) / 2.0D+00

  do i = 1, n
    gr = r0 * sqrt ( real ( i, kind = rk ) - 0.5D+00 )
    gt = 2.0D+00 * pi * real ( i, kind = rk ) / phi
    g(1,i) = c(1) + gr * cos ( gt )
    g(2,i) = c(2) + gr * sin ( gt )
  end do
...
!
!    Input, real ( kind = rk ) A(2,N), the vector to be printed.
!

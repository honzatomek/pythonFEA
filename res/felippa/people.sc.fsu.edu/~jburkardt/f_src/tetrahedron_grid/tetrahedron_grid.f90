...
!
!    Input, real ( kind = rk ) T(3,4), the vertices of the tetrahedron.
!
!    Input, integer NG, the number of grid points.
!
!    Output, real ( kind = rk ) TG(3,NG), the tetrahedron grid points.
!
  implicit none
  ...
  do i = 0, n
    do j = 0, n - i
      do k = 0, n - i - j
        l = n - i - j - k
        p = p + 1
        tg(1:3,p) = ( real ( i, kind = rk ) * t(1:3,1) &
                    + real ( j, kind = rk ) * t(1:3,2) &
                    + real ( k, kind = rk ) * t(1:3,3) &
                    + real ( l, kind = rk ) * t(1:3,4) ) / real ( n, kind = rk )
      end do
    end do
  end do

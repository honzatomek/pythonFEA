...
!
  ...
  call timestamp ( )
  write ( *, '(a)' ) ''
  write ( *, '(a)' ) 'CUBE_ARBQ_RULE_TEST'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Test CUBE_ARBQ_RULE().'
...
!
  ...
  real ( kind = rk ) rints(((degree+1)*(degree+2)*(degree+3))/6)
  real ( kind = rk ) volume
  real ( kind = rk ) w(n)
  real ( kind = rk ) x(3,n)
  real ( kind = rk ) z(3)

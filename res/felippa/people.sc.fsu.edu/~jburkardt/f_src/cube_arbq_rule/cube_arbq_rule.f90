...
!
!    Output, real ( kind = rk ) X(3,N), the quadrature nodes.
!
!    Output, real ( kind = rk ) W(N), the quadrature weights.
!
  implicit none
...
!  Parameters:
!
!    Input, real ( kind = rk ) N, the number of nodes.
!
!    Input, real ( kind = rk ) X(3,N), the coordinates of the nodes.
!
...
!    1 <= DEGREE <= 15.
!
!    Output, integer CUBE_ARBQ_SIZE, the number of points in the
!    corresponding rule.
!
  implicit none
...
!
!    Input, real ( kind = rk ) Z(3), the evaluation point.
!
!    Output, real ( kind = rk ) POLS(NPOLS), the polynomial values.
!
  implicit none
  ...
  real ( kind = rk ) f1(degree+1)
  real ( kind = rk ) f2(degree+1)
  real ( kind = rk ) f3(degree+1)
  integer kk
  integer m
  integer n1
  integer n2
  real ( kind = rk ) pols(((degree+1)*(degree+2)*(degree+3))/6)
  real ( kind = rk ) scale
  real ( kind = rk ) t
  real ( kind = rk ) z(3)
  ...
  do m = 0, degree
    do n2 = 0, m
      do n1 = 0, n2
        ...
        scale = 1.0D+00
        t = 0.5D+00 * real ( 1 + 2 * n1, kind = rk )
        scale = scale * sqrt ( t )
        t = 0.5D+00 * real ( 1 + 2 * n2 - 2 * n1, kind = rk )
        scale = scale * sqrt ( t )
        t = 0.5D+00 * real ( 1 + 2 * m - 2 * n2, kind = rk )
        scale = scale * sqrt ( t )
...
!
!    Output, real ( kind = rk ) POLS(DEGREE+1)), the orthogonal
...
!    1 <= I <= M.
!
!    Input, real ( kind = rk ) V(N), the row to be copied.
!
!    Input/output, real ( kind = rk ) A(M,N), the matrix into which
...
!
!    Output, real ( kind = rk ) X(3,N), the coordinates of the nodes.
!

...
!  Parameters:
!
!    Output, real ( kind = rk ) W(1), the weights.
!
!    Output, real ( kind = rk ) X(1), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(1) = (/ &
    1.0D+00 /)
  real ( kind = rk ) x(order)
  real ( kind = rk ) :: x_save(1) = (/ &
    0.0D+00 /)
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(2), the weights.
!
!    Output, real ( kind = rk ) X(2), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) w(order)
  real ( kind = rk ) :: w_save(2) = (/ &
    0.5D+00, &
    0.5D+00 /)
  real ( kind = rk ) x(order)
  real ( kind = rk ) :: x_save(2) = (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(3), the weights.
!
!    Output, real ( kind = rk ) X(3), the abscissas.
!
  implicit none
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(4), the weights.
!
!    Output, real ( kind = rk ) X(4), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) w(order)
  real ( kind = rk ) :: w_save(4) = (/ &
    0.173927422568727D+00, &
    0.326072577431273D+00, &
    0.326072577431273D+00, &
    0.173927422568727D+00 /)
  real ( kind = rk ) x(order)
  real ( kind = rk ) :: x_save(4) = (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(5), the weights.
!
!    Output, real ( kind = rk ) X(5), the abscissas.
!
  implicit none
...
!    Output, real ( kind = rk ) W(1), the weights.
!
!    Output, real ( kind = rk ) XY(2,1), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(1) = (/ &
    1.0D+00 /)
  real ( kind = rk ) xy(2,order)
  real ( kind = rk ) :: xy_save(2,1) = reshape ( (/ &
...
!    Output, real ( kind = rk ) W(3), the weights.
!
!    Output, real ( kind = rk ) XY(2,3), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(3) = (/ &
    ...
    0.33333333333333333333D+00 /)
  real ( kind = rk ) xy(2,order)
  real ( kind = rk ) :: xy_save(2,3) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(6), the weights.
!
!    Output, real ( kind = rk ) XY(2,6), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(6) = (/ &
    ...
    0.10995174365532186764D+00 /)
  real ( kind = rk ) xy(2,order)
  real ( kind = rk ) :: xy_save(2,6) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(7), the weights.
!
!    Output, real ( kind = rk ) XY(2,7), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) xy(2,order)
  real ( kind = rk ) :: xy_save(2,7) = reshape ( (/ &
...
!  Parameters:
!
!    Output, real ( kind = rk ) W(12), the weights.
!
!    Output, real ( kind = rk ) XY(2,12), the abscissas.
!
  implicit none
  ...
  real ( kind = rk ) :: w_save(12) = (/ &
     ...
     0.082851075618373575194D+00 /)
  real ( kind = rk ) xy(2,order)
  real ( kind = rk ) :: xy_save(2,12) = reshape ( (/ &
...
!
  if ( expon(3) == - 1 ) then
    ...
    value = value * 2.0D+00 / real ( expon(3) + 1, kind = rk )
  end if
...
!    1 <= LINE_ORDER <= 5.
!
!    Input, integer TRIANGLE_ORDER, the indes of the triangle rule.
!    The index of the rule is 1, 3, -3, 6, -6, 7 or 12.
!
!    Output, real ( kind = rk ) W(LINE_ORDER*abs(TRIANGLE_ORDER)), the weights.
!
!    Output, real ( kind = rk ) XYZ(3,LINE_ORDER*abs(TRIANGLE_ORDER)), 
!    the abscissas.
!
  implicit none
  ...
  integer j
  integer k
  real ( kind = rk ) line_w(line_order)
  real ( kind = rk ) line_x(line_order)
  real ( kind = rk ) triangle_w(abs(triangle_order))
  real ( kind = rk ) triangle_xy(2,abs(triangle_order))
  real ( kind = rk ) w(line_order*abs(triangle_order))
  real ( kind = rk ) xyz(3,line_order*abs(triangle_order))
...
!  Parameters:
!
!    Output, real ( kind = rk ) WEDGE_VOLUME, the volume.
!
  implicit none

  integer, parameter :: rk = kind ( 1.0D+00 )

  real ( kind = rk ) wedge_volume

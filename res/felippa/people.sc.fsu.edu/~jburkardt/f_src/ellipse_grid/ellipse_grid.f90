...
!
!    Input, real ( kind = rk ) R(2), the half axis lengths.
!
!    Input, real ( kind = rk ) C(2), the center of the ellipse.
!
!    Input, integer ( kind = rk ) NG, the number of grid points 
!    inside the ellipse.
!
!    Output, real ( kind = rk ) XY(2,NG), the grid points.
!
  implicit none

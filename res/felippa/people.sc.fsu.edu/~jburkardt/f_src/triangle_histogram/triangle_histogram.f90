...
!
  ...
  integer data_num
  real ( kind = rk ), allocatable :: data_xy(:,:)
  integer dim_num
  integer, allocatable :: histo(:)
  real ( kind = rk ) histo_ave
  integer histo_max
  integer histo_min
  real ( kind = rk )  histo_std
  integer i
  integer iarg
  integer ierror
  integer j
  ...
  call timestamp ( )
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM:'
  write ( *, '(a)' ) '  FORTRAN90 version.'
  write ( *, '(a)' ) '  Compute a histogram for data in the reference triangle.'
!
...
!
!  Second argument is N, which determines the number of subtriangles.
!
  if ( 2 <= arg_num ) then
    ...
    write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM:'
    write ( *, '(a)' ) &
      '  Please enter N = 1, 2, 3, ..., the order of refinement.'
...
!
  ...
  if ( dim_num /= 2 ) then
    write ( *, * ) ' '
    write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM - Fatal error!'
...
!
  do d = 1, data_num
    if ( data_xy(1,d) < 0.0D+00 .or. &
         data_xy(2,d) < 0.0D+00 .or. &
                        1.0D+00 < data_xy(1,d) + data_xy(2,d) ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'TRIANGLE_HISTOGRAM - Fatal error!'
      write ( *, '(a,i8,a)' ) &
        '  Point ', d, ' does not lie inside the triangle.'
      write ( *, '(a,g14.6)' ) '  X = ', data_xy(1,d)
      write ( *, '(a,g14.6)' ) '  Y = ', data_xy(2,d)
      stop
    end if
  end do
!
...
!
  do d = 1, data_num
    ...
    j = int (                            data_xy(2,d)   &
      * real ( n, kind = rk ) ) + 1
    k = int ( ( 1.0D+00 - data_xy(1,d) - data_xy(2,d) ) &
      * real ( n, kind = rk ) ) + 1
...
!
!  Convert from (I,J) indexing to a linear index K,
!  and increment the histogram vector.
!
    histo(t) = histo(t) + 1
...
!
  histo_ave = real ( sum ( histo(1:sub_num) ), kind = rk ) &
    / real ( sub_num, kind = rk )
  histo_max = maxval ( histo(1:sub_num) )
  histo_min = minval ( histo(1:sub_num) )
  call i4vec_std ( sub_num, histo, histo_std )
!
!  Print the histogram.
!
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Data from file "' // trim ( data_filename ) // '".'
  write ( *, '(a,i8)' ) '  Number of points = ', data_num
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Triangle refinement level N = ', n
  write ( *, '(a,i8)' ) '  Number of triangles = ', sub_num
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Counting number of points in each subtriangle:'
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Minimum = ', histo_min
  write ( *, '(a,g14.6)' ) '  Average = ', histo_ave
  write ( *, '(a,i8)' ) '  Maximum = ', histo_max
  write ( *, '(a,g14.6)' ) '  STD     = ', histo_std
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '     COUNT'
  write ( *, '(a)' ) ' '
  do k = 1, n * n
  ...
  write ( *, * ) ' '
  write ( *, * ) '  Number of out-or-range points = ', histo(n*n+1)
!
!  Free memory.
!
  deallocate ( data_xy )
...
!
!    Output, real ( kind = rk ) STD, the standard deviation of the entries.
!
  implicit none
  ...
  std = 0.0D+00
  do i = 1, n
    std = std + ( real ( a(i), kind = rk ) - mean )**2
  end do

  if ( 1 < n ) then
    std = sqrt ( std / real ( n - 1, kind = rk ) )
  else
    std = 0.0D+00
  end if

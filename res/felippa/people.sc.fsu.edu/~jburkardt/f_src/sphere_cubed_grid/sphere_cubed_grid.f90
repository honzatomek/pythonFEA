...
!
  ...
  if ( i == 0 ) then
    ...
    xc = tan ( real ( 2 * i - n, kind = rk ) * 0.25D+00 * r8_pi &
      / real ( n, kind = rk ) )
  end if
  ...
  if ( j == 0 ) then
    ...
    yc = tan ( real ( 2 * j - n, kind = rk ) * 0.25D+00 * r8_pi &
      / real ( n, kind = rk ) )
  end if
  ...
  if ( k == 0 ) then
    ...
    zc = tan ( real ( 2 * k - n, kind = rk ) * 0.25D+00 * r8_pi &
      / real ( n, kind = rk ) )
  end if
...
!
!    Output, real ( kind = rk ) LINE_DATA(3,2,LINE_NUM), for each line I, the 
...
!
  ...
  integer j
  integer l
  real ( kind = rk ) line_data(3,2,line_num)
  integer n
...
!
!    Input, real ( kind = rk ) XG(3,NG), the Fibonacci spiral points.
!
!    Input, integer LINE_NUM, the number of grid lines.
!
!    Input, real ( kind = rk ) LINE_DATA(3,2,LINE_NUM), contains pairs of 

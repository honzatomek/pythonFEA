...
!
  ...
  do i = 1, 3
    ...
    value = value * ( b(i) ** ep1 - a(i) ** ep1 ) / real ( ep1, kind = rk )
...
!
!    Output, real ( kind = rk ) W(ORDER_1D(1)*ORDER_1D(2)*ORDER_1D(3)), 
!    the weights.
!
!    Output, real ( kind = rk ) XYZ(3,ORDER_1D(1)*ORDER_1D(2)*ORDER_1D(3)), 
!    the abscissas.
!
  implicit none
  ...
  integer order
  integer order_1d(3)
  real ( kind = rk ) w(order_1d(1)*order_1d(2)*order_1d(3))
  real ( kind = rk ), allocatable :: w_1d(:)
  real ( kind = rk ), allocatable :: x_1d(:)
  real ( kind = rk ) xyz(3,order_1d(1)*order_1d(2)*order_1d(3))
...
!    Input, real ( kind = rk ) A(3), B(3), the lower and upper limits.
!
!    Output, real ( kind = rk ) CUBE_UNIT_VOLUME, the volume.
!
  implicit none
...
!  Local:
!
!    integer START, the first location of a block of values
!    to set.
!
!    integer CONTIG, the number of consecutive values 
!    to set.
!
!    integer SKIP, the distance from the current value of START
!    to the next location of a block of values to set.

...
!      sum ( X(1:N) ) <= T
!
!    where T is an integer ( kind = 4 ).
!
!    Lattice points are points X which satisfy the simplex conditions and
!    for which all the components are integer ( kind = 4 )s.
!
...
!    N must be positive.
!
!    Input, integer ( kind = 4 ) T, the characteristic of the simplex.
...
!
!    Input/output, integer ( kind = 4 ) X(N), not initialized by the user, but 
!    not changed by the user on subsequent calls.  The routine returns

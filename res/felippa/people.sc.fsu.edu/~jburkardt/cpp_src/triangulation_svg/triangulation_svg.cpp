...
void timestamp ( );
void triangulation_plot ( string filename, int node_num, double node_xy[],
...
//
{
  ...
  timestamp ( );
  cout << "\n";
  cout << "TRIANGULATION_SVG\n";
  cout << "  C++ version:\n";
  cout << "  Make an SVG plot of triangulated data.\n";
  cout << "\n";
  cout << "  This program expects two files:\n";
  cout << "  * prefix_nodes.txt,    node coordinates,\n";
  cout << "  * prefix_elements.txt, indices of nodes forming elements,\n";
  cout << "  and creates:\n";
  cout << "  * prefix.svg, an SVG image of the triangulation.\n";
//
...
//
  ...
  {
    x_max = fmax ( x_max, node_xy[0+j*2] );
    x_min = fmin ( x_min, node_xy[0+j*2] );
  }
  x_scale = x_max - x_min;
  ...
  for ( j = 0; j < node_num; j++ )
  {
    y_max = fmax ( y_max, node_xy[1+j*2] );
    y_min = fmin ( y_min, node_xy[1+j*2] );
  }
  y_scale = y_max - y_min;
...
//
  ...
  output << "  version=\"1.1\">\n";
  output << "  <desc>\n";
  output << "    Triangulation created by triangulation_svg.cpp\n";
  output << "  </desc>\n";
  ...
  cout << "\n";
  cout << "  Graphics data written to file \"" << filename << "\"\n";

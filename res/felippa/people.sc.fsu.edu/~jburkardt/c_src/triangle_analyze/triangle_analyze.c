...
int triangle_contains_point_2d ( double t[2*3], double p[2] );
...
/*
  ANGLES
*/
  angles = triangle_angles_2d_new ( node_xy );

  r8vec_print ( 3, angles, "  ANGLES (radians):" );

  for ( i = 0; i < 3; i++ )
  {
    angles[i] = r8_degrees ( angles[i] );
  }
...
*/
  ...
  printf ( "  CIRCUM_CENTER: %g  %g\n", circum_center[0], circum_center[1] );
/*
  EDGE LENGTHS
*/
...
*/
  ...
  printf ( "\n" );
  printf ( "  IN_RADIUS: %g\n", in_radius );
  printf ( "  IN_CENTER: %g  %g\n", in_center[0], in_center[1] );
/*
  ORIENTATION
*/
  ...
  if ( orientation == 0 )
  {
    printf ( "  ORIENTATION: CounterClockwise.\n" );
  }
  else if ( orientation == 1 )
  {
    printf ( "  ORIENTATION: Clockwise.\n" );
  }
  else if ( orientation == 2 )
  {
    printf ( "  ORIENTATION: Degenerate Distinct Colinear Points.\n" );
  }
  else if ( orientation == 3 )
  {
    printf ( "  ORIENTATION: Degenerate, at least two points identical.\n" );
  }
/*
  ORTHO_CENTER
*/
...
/*
  ...
  Parameters:
    ...
    Output, int TRIANGLE_CONTAINS_POINT_2D, is TRUE if P is inside

...
/*
  ...
  Parameters:
    ...
    Output, double LINE_EXP_PERP[2], a point on the given line, such that 
...
*/
  ...
  {
    fprintf ( stderr, "\n" );
    fprintf ( stderr, "LINE_EXP2IMP - Fatal error!\n" );
...
int line_imp_is_degenerate ( double a, double b, double c )
...
/*
  ...
  Parameters:
    ...
    Output, int LINE_IMP_IS_DEGENERATE, is true if the
    line is degenerate.
*/
{
  int value;
...
int triangle_contains_point ( double t[2*3], double p[2] )
...
/*
  ...
  Parameters:
    ...
    Output, int TRIANGLE_CONTAINS_POINT, is TRUE if P is inside
...
void triangle_incircle ( double t[2*3], double pc[2], double *r )
...
void triangle_orthocenter ( double t[2*3], double p[2], int *flag )
...
*/
{
  ...
  value = fmin ( value, segment_point_dist ( t+1*2, t+2*2, p ) );
  value = fmin ( value, segment_point_dist ( t+2*2, t+0*2, p ) );
...
/*
  ...
  Parameters:
    ...
    Output, double TRIANGLE_REFERENCE_SAMPLE[2*N], random points in the triangle.
*/
{
  double alpha;
  double beta;
...
*/
{
  ...
  xsi = ( double * ) malloc ( 3 * sizeof ( double ) );

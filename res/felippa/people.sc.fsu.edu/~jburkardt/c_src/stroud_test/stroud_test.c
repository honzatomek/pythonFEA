...
*/
{
  ...
  if ( d <= p )
  {
    ...
    volume_1d = sqrt ( pi ) * tgamma ( alpha + 1.0 )
      / tgamma ( alpha + 1.5 );
    x = ( double * ) malloc ( n * o * sizeof ( double ) );
    w = ( double * ) malloc ( o * sizeof ( double ) );
...
*/
{
  ...
  if ( d <= p )
  {
    ...
    c1 = 2.0 * ( alpha + 1.0 ) * ( beta + 1.0 ) / ( alpha + beta + 3.0 )
      / ( alpha + beta + 2.0 );
    volume_1d = pow ( 2.0, alpha + beta + 1.0 ) * tgamma ( alpha + 1.0 )
      * tgamma ( beta + 1.0 ) / ( alpha + beta + 1.0 ) / tgamma ( alpha + beta + 1.0 );
    x = ( double * ) malloc ( n * o * sizeof ( double ) );
    w = ( double * ) malloc ( o * sizeof ( double ) );
    gw_02_xiu ( n, o, gamma0, delta0, c1, volume_1d, x, w );
    v = monomial_value ( n, o, x, expon );

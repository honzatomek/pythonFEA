...
!
!    Input, integer ( kind = 4 ) NMESSG, the actual number of characters in
!    MESSG.  If NMESSG is 0, no message is being supplied.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) MAXMES, the maximum number of times any one
...
!
!    Input, integer ( kind = 4 ) NERR, the error number, chosen by the library
!    routine's author.  It must be in the range -99 to 999 (three printable 
!    digits).  Each distinct error should have its own error number.  These 
!    error numbers should be described in the machine readable documentation
...
!
!    Input, integer ( kind = 4 ) LEVEL, a value in the range 0 to 2 that 
!    indicates the level (severity) of the error.  Their meanings are
...
!
!    Input, integer ( kind = 4 ) NR, the number of real values to be printed. 
!    (0 to 2)
!
...
!  Parameters:
!
!    Output, integer ( kind = 4 ) IUNIT, the logical unit number of the first
!    unit to which error messages are being sent.
!
  implicit none
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) IUNIT(NUNIT), unit numbers to which the error
!    messages should be printed.  Normally these numbers should all be 
!    different but duplicates are not prohibited.
!
!    Input, integer ( kind = 4 ) NUNIT, the number of unit numbers provided
!    in IUNIT.  1 <= N <= 5.
!
  implicit none

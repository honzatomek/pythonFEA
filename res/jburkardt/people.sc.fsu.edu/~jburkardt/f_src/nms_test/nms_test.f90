...
subroutine alngam_test ( )

!*****************************************************************************80
!
!! alngam_test() tests alngam().
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
  ...
  real ( kind = rk ), parameter :: alpha = 0.0D+00
  real ( kind = rk ) fx
  real ( kind = rk ) fx2(1)
  integer n
  integer nz
...
!
  ...
  real ( kind = rk ), parameter :: alpha = 1.0D+00
  real ( kind = rk ) fx
  real ( kind = rk ) fx2(1)
  integer n
  integer nz
...
!
!    Output, real ( kind = rk ) FVEC(N), the function values at X, if IFLAG = 1.
!
...
!  Normalize the data
!
  r(1:n) = r(1:n) / ( 2.0D+00 * real ( n - 1, kind = rk ) )
...
!
  ...
  real ( kind = rk ), parameter :: a = -3.0D+00
  real ( kind = rk ), parameter :: b = 3.0D+00
  real ( kind = rk ) dnor
  real ( kind = rk ) dstart
  integer h(nbins)
  integer i
  ...
  iseed = 305
  rseed = dstart ( iseed )
  width = ( b - a ) / real  ( nbins - 2, kind = rk )
...
!
  do i = 0, nstep
    ...
    tout = real ( 2 * i, kind = rk ) * pi / real ( nstep, kind = rk )
...
!
  ...
  real ( kind = rk ) ewt
  external fsub
  real ( kind = rk ), external :: gfun
  real ( kind = rk ), parameter :: h = 10.0D+00
  integer iw(liw)
  real ( kind = rk ), parameter :: mass = 0.125D+00
...
!
  ...
  real ( kind = rk ), parameter :: g = 32.0D+00
  real ( kind = rk ) t
  real ( kind = rk ) y(n+1)
  real ( kind = rk ) ydot(n)
...
!
  ...
  call r8_fake_use ( fjac(1,1) )
  call r8_fake_use ( fvec(1) )
  call i4_fake_use ( iflag )
...
!
  ...
  integer m
  integer n
  real ( kind = rk ) qraux(nn)
  real ( kind = rk ) tol
  real ( kind = rk ) work(nn)
  real ( kind = rk ) x(nn)
!
...
!
  ...
  integer info
  integer j
  real ( kind = rk ), dimension ( n ) :: pop = (/ 75.994575D+00, 91.972266D+00, &
...
!
  ...
  do i = 1, 8
    y(i) = 1900.0D+00 + real ( ( i - 1 ) * 10, kind = rk )
  end do
...
!
  ...
  do i = 1, 9
    ...
    year = 1900.0D+00 + real ( i - 1, kind = rk ) * 10.0D+00
...
!
  ...
  real ( kind = rk ) a(2*n)
  real ( kind = rk ) acovr(0:2*n-1)
  real ( kind = rk ) azero
  real ( kind = rk ) b(2*n)
  real ( kind = rk ) el(0:2*n-1)
  real ( kind = rk ) el_sum
  logical ex
  integer i
  real ( kind = rk ) wsave(4*(2*n)+15)
...
!
  el(0:n-1) = el(0:n-1) - el_sum / real ( n, kind = rk )
...
!
  ...
  real ( kind = rk ) a((mcoef+1)/2)
  real ( kind = rk ) azero
  real ( kind = rk ) b((mcoef+1)/2)
  real ( kind = rk ) c((mcoef+1)/2)
  logical, parameter :: debug = .true.
  real ( kind = rk ) del
  real ( kind = rk ) dfta((mcoef+1)/2)
  real ( kind = rk ) dftb((mcoef+1)/2)
  real ( kind = rk ) error
  real ( kind = rk ) f
  integer j
  integer k
  ...
  integer n
  real ( kind = rk ) :: pi = 3.141592653589793D+00
  real ( kind = rk ) r(mcoef)
  real ( kind = rk ), external :: runge
  real ( kind = rk ) s((mcoef+1)/2)
  real ( kind = rk ) tn
  real ( kind = rk ) wsave(3*mcoef+15)
  real ( kind = rk ) x
  real ( kind = rk ) x0
  real ( kind = rk ) xj
...
!
    del = 2.0D+00 / real ( n, kind = rk )
    f = 2.0D+00 * pi / ( real ( n, kind = rk ) * del )
!
...
!
    if ( debug ) then
      ...
      do k = 1, m
        ...
        x = - 1.0D+00 + 2.0D+00 * ( k - 1.0D+00 ) / real ( m - 1, kind = rk )
...
!
  do i = 1, n
    x(i) = -1.0D+00 + real ( i - 1, kind = rk ) / 10.0D+00
    f(i) = runge ( x(i) )
  end do
!
...
!
  do i = 1, ne
    xe(i) = -1.0D+00 + real ( i - 1, kind = rk ) / real ( ne - 1, kind = rk )
  end do
...
!
  ...
  real ( kind = rk ) eps_x
  real ( kind = rk ) err
  real ( kind = rk ), external :: g13
  integer iflag
  integer kf
  real ( kind = rk ) result
  real ( kind = rk ) x_fixed
...
!
  ...
  real ( kind = rk ), parameter :: ahi = 5.0D+00
  real ( kind = rk ), parameter :: alo = 0.0D+00
  real ( kind = rk ) c(n)
  integer i
...
!  Set the data values.
!
  call random_number ( harvest = c(1:n) )
...
!  original data.
  ...
  write ( *, '(a)' ) ' '
  do i = 1, 10
    write ( *, '(2x,g14.6)' ) c(i) / real ( 2 * ( n + 1 ), kind = rk )
  end do
...
!
  ...
  real ( kind = rk ), parameter, dimension ( m ) :: b = &
    (/ 20.0D+00, 9.0D+00, 3.0D+00, 1.0D+00 /)
  real ( kind = rk ) f
  integer j
  real ( kind = rk ), parameter, dimension ( m ) :: t = &
...
!
  ...
  do i = 1, n
    x0(i) = real ( i, kind = rk ) / real ( n + 1, kind = rk )
  end do
!
...
!
  ...
  integer, parameter :: city_num = 4
  integer, parameter :: n = 2 * city_num
  integer, parameter :: lwork = n*(n+10)

  real ( kind = rk ) d(city_num,city_num)
  real ( kind = rk ) f
  integer i
  integer ierror
  integer j
...
!
  ...
  integer city2
  real ( kind = rk ) d
  real ( kind = rk ) :: distance(4,4) = reshape ( (/ &
...
!
  ...
  integer, parameter :: ck = kind ( ( 1.0D+00, 2.0D+00 ) )
...
!
  ...
  real ( kind = rk ) acov(0:n-1)
  complex ( kind = ck ) cel(0:2*n-1)
  complex ( kind = ck ) corr(0:2*n-1)
  real ( kind = rk ) el(0:2*n-1)
  real ( kind = rk ) el_sum
  logical ex
  integer i
...
!
  cel(0:2*n-1) = cmplx ( el(0:2*n-1), 0.0D+00, kind = ck )
!
...
!
  corr(0:2*n-1) = abs ( cel(0:2*n-1) / real ( 2 * n, kind = rk ) ) **2
!
...
!
  ...
  acov(0:n-1) = real ( corr(0:n-1), kind = rk ) * real ( 2 * n, kind = rk )
!
...
!
  ...
  real ( kind = rk ) err
  integer i
  complex ( kind = ck ) image(lda,n)
  complex ( kind = ck ) image2(lda,n)
  integer j
  real ( kind = rk ) wsave(4*n+15)
...
!
  do i = 1, n
    do j = 1, n
      ...
      image(i,j) = cmplx ( a(i,j), 0.0D+00, kind = ck )
...
!
  image(1:n,1:n) = image(1:n,1:n) / real ( n**2, kind = rk )
!
...
!
  ...
  real ( kind = rk ) :: pi = 3.141592653589793D+00
  real ( kind = rk ), external :: runge
  complex ( kind = ck ) sqtm1
  real ( kind = rk ) wsave(150)
  real ( kind = rk ) x0
  real ( kind = rk ) xj

  sqtm1 = cmplx ( 0.0D+00, -1.0D+00, kind = rk )
...
!
    do j = 0, n-1
      xj = -1.0D+00 + real ( j, kind = rk ) * del
      coeff(j) = cmplx ( runge(xj), 0.0D+00, kind = ck )
    end do

...
!
!    Input/output, real ( kind = rk ) Y(NEQN), the current solution.
!
!    Input/output, real ( kind = rk ) T, the current value of the independent
!    variable.
!
!    Input, real ( kind = rk ) TOUT, the desired value of T on output.
!
!    Input, real ( kind = rk ) RELERR, ABSERR, the relative and absolute error
!    tolerances.  At each step, the code requires
!      abs ( local error ) <= abs ( y ) * relerr + abserr
!    for each component of the local error and solution vectors.
!
!    Input/output, integer IFLAG, indicates the status of 
...
!
!    Input/output, real ( kind = rk ) WORK(100+21*NEQN), workspace.
!
!    Input/output, integer IWORK(5), workspace.
!
  implicit none
  ...
  real ( kind = rk ) abserr
  external f
  integer, parameter :: ialpha = 1
  integer, parameter :: ibeta = 13
  integer, parameter :: idelsn = 93
  integer iflag
  integer, parameter :: ig = 62
  integer, parameter :: ih = 89
  integer, parameter :: ihold = 90
  integer ip
  integer, parameter :: iphase = 75
  integer iphi
  integer, parameter :: ipsi = 76
  integer, parameter :: isig = 25
  integer, parameter :: istart = 91
  integer, parameter :: itold = 92
  integer, parameter :: iv = 38
  integer, parameter :: iw = 50
  integer iwt
  integer, parameter :: ix = 88
  integer iyp
  integer iypout
  integer, parameter :: iyy = 100
  integer iwork(5)
  logical nornd
  logical phase1
  real ( kind = rk ) relerr
...
!
!    Workspace, real ( kind = rk ) YY(NEQN), used to hold old solution data.
!
!    Input, real ( kind = rk ) WT(NEQN), the error weight vector.
!
!    Workspace, real ( kind = rk ) P(NEQN).
!
!    Workspace, real ( kind = rk ) YP(NEQN), used to hold values of the
!    solution derivative.
!
!    Workspace, real ( kind = rk ) YPOUT(NEQN), used to hold values of the
!    solution derivative.
!
!    Workspace, real ( kind = rk ) PHI(NEQN,16), contains divided difference
!    information about the polynomial interpolant to the solution.
!
!    Workspace, real ( kind = rk ) ALPHA(12), BETA(12), SIG(13).
!
!    Workspace, real ( kind = rk ) V(12), W(12), G(13).
!
...
!
!    Workspace, real ( kind = rk ) PSI(12), contains information about
!    the polynomial interpolant to the solution.
!
!    Input/output, real ( kind = rk ) X, a "working copy" of T, the current value
!    of the independent variable, which is adjusted as the code attempts
!    to take a step.
!
!    Input/output, real ( kind = rk ) H, the current stepsize.
!
!    Input/output, real ( kind = rk ) HOLD, the last successful stepsize.
!
!    Input/output, logical START, is TRUE on input for the first step.
!    The program initializes data, and sets START to FALSE.
!
!    Input/output, real ( kind = rk ) TOLD, the previous value of T.
!
!    Input/output, real ( kind = rk ) DELSGN, the sign (+1 or -1) of
!    TOUT - T.
!
!    Input/output, integer NS, the number of steps taken with 
!    stepsize H.
!
!    Input/output, logical NORND, ?
!
!    Input, integer K, the order of the current ODE method.
!
!    Input, integer KOLD, the order of the ODE method on the 
!    previous step.
!
!    Input/output, integer ISNOLD, the previous value of ISN, the 
!    sign of IFLAG.
!
!  Local:
!
...
!
  ...
  real ( kind = rk ) abseps
  real ( kind = rk ) abserr
  real ( kind = rk ) alpha(12)
  real ( kind = rk ) beta(12)
  logical crash
  real ( kind = rk ) del
  real ( kind = rk ) delsgn
  real ( kind = rk ) eps
  ...
  real ( kind = rk ) v(12)
  real ( kind = rk ) w(12)
  real ( kind = rk ) wt(neqn)
  real ( kind = rk ) x
  real ( kind = rk ) y(neqn)
  real ( kind = rk ) yp(neqn)
  real ( kind = rk ) ypout(neqn)
  real ( kind = rk ) yy(neqn)
!
...
!  Parameters:
!
!    Input/output, real ( kind = rk ) X, the value of the independent variable.
!
!    Input/output, real ( kind = rk ) Y(NEQN), the approximate solution at the
...
!    Input, integer NEQN, the number of equations.
!
!    Input/output, real ( kind = rk ) H, the suggested stepsize.
!
!    Input/output, real ( kind = rk ) EPS, the local error tolerance.
!
!    Input, real ( kind = rk ) WT(NEQN), the vector of error weights.
!
!    Input/output, logical START, is set to TRUE before the first step.
!    The program initializes data, and resets START to FALSE.
!
!    Input/output, real ( kind = rk ) HOLD, the step size used on the last
!    successful step.
!
!    Input/output, integer K, the appropriate order for the 
!    next step.
!
!    Input/output, integer KOLD, the order used on the last
!    successful step.
!
...
!
  ...
  real ( kind = rk ) fouru
  real ( kind = rk ) g(13)
  real ( kind = rk ), dimension ( 13 ) :: gstr = (/ &
...
!
    if ( ns <= k ) then
      ...
      beta(ns) = 1.0D+00
      alpha(ns) = 1.0D+00 / real ( ns, kind = rk )
      temp1 = h * real ( ns, kind = rk )
      sig(nsp1) = 1.0D+00
      ...
      do i = nsp1, k
        ...
        temp1 = temp2 + h
        alpha(i) = h / temp1
        sig(i+1) = real ( i, kind = rk ) * alpha(i) * sig(i)
      end do
...
!
      if ( ns <= 1 ) then
        ...
        do iq = 1, k
          v(iq) = 1.0D+00 / real ( iq * ( iq + 1 ), kind = rk )
          w(iq) = v(iq)
        end do
!
...
!
        ...
        if ( kold < k ) then
          ...
          v(k) = 1.0D+00 / real ( k * kp1, kind = rk )
...
!  Parameters:
!
!    Input, real ( kind = rk ) X, the point where the solution has been computed.
!
!    Input, real ( kind = rk ) Y(NEQN), the computed solution at X.
!
!    Input, real ( kind = rk ) XOUT, the point at which the solution is desired.
!
!    Output, real ( kind = rk ) YOUT(NEQN), the solution at XOUT.
!
!    Output, real ( kind = rk ) YPOUT(NEQN), the derivative of the solution
!    at XOUT.
!
!    Input, integer NEQN, the number of equations.
!
!    Input, integer KOLD, the order used for the last
!    successful step.
!
!    Input, real ( kind = rk ) PHI(NEQN,16), contains information about the
!    interpolating polynomial.
!
!    Input, real ( kind = rk ) PSI(12), contains information about the
!    interpolating polynomial.
!
  implicit none

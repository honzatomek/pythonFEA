midpoint <- function ( f, t0, tstop, y0, n, ... ) 

#*****************************************************************************80
#
## midpoint() implements the implicit midpoint ODE solver.
#
#  Licensing:
#
...
#
{
  ...
  else if ( is.matrix(y0) )
  {
    if ( ncol(y0) != 1 ) 
    {
      stop ( "midpoint: Argument 'y0' must be a row or column vector.")
    }
  }
...
#
  mmfun <- function(w) w - y - 0.5 * dt * f ( tw, w )

  f_length <- length ( f(t0, y0) )
  if ( f_length != y_length )
  {
    stop ( "midpoint: Function f must return a vector the same length as 'y0'." )
  }
#

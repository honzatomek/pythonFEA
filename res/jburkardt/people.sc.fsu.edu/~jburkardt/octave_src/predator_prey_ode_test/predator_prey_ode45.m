function predator_prey_ode45 ( tspan, p0 )

%*****************************************************************************80
%
%% predator_prey_ode45 solves the predator-prey system using ode45().
%
%  Discussion:
%
...
%    There is a conserved quantity, which here would be:
 %     h = 0.002 * r + 0.001 * f - 10.0 * log ( r ) - 2.0 * log ( f );
% 
%  Licensing:
%
...
%
  ...
  xlabel ( 'Time' );
  ylabel ( 'Population' );
  title ( 'ode45: Predator Prey Time Plot' )
  filename = 'predator_prey_ode45_plot.png';
  print ( '-dpng', filename );
  ...
  xlabel ( 'Rabbits' );
  ylabel ( 'Foxes' );
  title ( 'ode45: Predator Prey Phase Plot' )
  filename = 'predator_prey_ode45_phase.png';
  print ( '-dpng', filename );
  fprintf ( 1, '  Graphics saved as "%s"\n', filename );
  ...
  title ( 'ode45: H(T) conservation', 'fontsize', 24 );
  filename = 'predator_prey_ode45_conserved.png';
  print ( '-dpng', filename );
  fprintf ( 1, '  Graphics saved as "%s"\n', filename );

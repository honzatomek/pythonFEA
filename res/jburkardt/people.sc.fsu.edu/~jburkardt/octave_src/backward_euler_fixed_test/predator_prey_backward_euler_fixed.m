function predator_prey_backward_euler_fixed ( tspan, p0, n )

%*****************************************************************************80
%
%% predator_prey_backward_euler_fixed(): solve using backward_euler_fixed().
%
%  Discussion:
%
...
%
  fprintf ( 1, '\n' );
  fprintf ( 1, 'predator_prey_backward_euler_fixed()\n' );
  fprintf ( 1, '  A pair of ordinary differential equations for a population\n' );
  fprintf ( 1, '  of predators and prey are solved using the backward Euler\n' );
  fprintf ( 1, '  method, solving the implicit function with a fixed point iteration.\n' );
  fprintf ( 1, '\n' );
  fprintf ( 1, '  The exact solution shows periodic behavior, with a fixed\n' );
  fprintf ( 1, '  period and amplitude.\n' );

  f = @ predator_prey_deriv;
  [ t, pout ] = backward_euler_fixed ( f, tspan, p0, n );
%
%  Plot the solution.
%
  ...
  xlabel ( 'Rabbits' );
  ylabel ( 'Foxes' );
  title ( 'predator prey backward euler fixed: phase' )

function [ f1, f2, f3, f4, f5, s ] = fehl ( f, neqn, y, t, h, yp )
...
%    distinguished.
%
%    Note that the "transpose operator" denoted by ' is actually a
%    complex conjugate transpose operator, which makes a big difference
%    if your data is complex.  Therefore, usage of ' has been corrected
%    by replacement with the transpose() function.
%
%  Licensing:
%
...
%
%    real Y(NEQN): the current value of the
%    dependent variable.
%
%    real T: the current value of the independent
%    variable.
%
%    real H: the step size to take.
%
%    real YP(NEQN): the current value of the derivative of 
...
%
  ...
  f5(1:neqn) = y(1:neqn) + ch * transpose ( yp(1:neqn) );

  f1 = transpose ( f ( t + ch, f5 ) );

  ch = 3.0 * h / 32.0;

  f5(1:neqn) = y(1:neqn) + ch * ( transpose ( yp(1:neqn) ) + 3.0 * f1(1:neqn) );

  f2 = transpose ( f ( t + 3.0 * h / 8.0, f5 ) );

  ch = h / 2197.0;

  f5(1:neqn) = y(1:neqn) + ch * ...
  ( 1932.0 * transpose ( yp(1:neqn) ) ...
  + ( 7296.0 * f2(1:neqn) - 7200.0 * f1(1:neqn) ) ...
  );

  f3 = transpose ( f ( t + 12.0 * h / 13.0, f5 ) );

  ch = h / 4104.0;

  f5(1:neqn) = y(1:neqn) + ch * ...
  ( ...
    ( 8341.0 * transpose ( yp(1:neqn) ) - 845.0 * f3(1:neqn) ) ...
  + ( 29440.0 * f2(1:neqn) - 32832.0 * f1(1:neqn) ) ...
  );

  f4 = transpose ( f ( t + h, f5 ) );

  ch = h / 20520.0;

  f1(1:neqn) = y(1:neqn) + ch * ...
  ( ...
    ( -6080.0 * transpose ( yp(1:neqn) ) ...
    + ( 9295.0 * f3(1:neqn) - 5643.0 * f4(1:neqn) ) ...
    ) ...
  + ( 41040.0 * f1(1:neqn) - 28352.0 * f2(1:neqn) ) ...
  );

  f5 = transpose ( f ( t + h / 2.0, f1 ) );
%
